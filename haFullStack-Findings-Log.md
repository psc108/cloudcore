...
### F-099 — The OpenTofu provider's own `flavor` attribute validator hard-coded the original flavor names, rejecting a new one `api/compute.py` already knew about

**Where:** the provider's `instance.go` and `nfs_server.go`, each with their own `stringvalidator.OneOf(...)` list for the `flavor` attribute.

**Symptom:** found live testing a new `standard.xlarge` flavor (6 vCPU/8GB, added to `api/compute.py`'s own `FLAVORS` dict for `examples/llm-chat`'s worker sizing) — `tofu plan`/`apply` rejected it outright with a provider validation error, even though the backend API itself already recognized the flavor name fine.

**Root cause:** the provider's own flavor list is a separate, hand-maintained string constant in Go, not derived from `api/compute.py`'s `FLAVORS` dict — adding a flavor to one side doesn't propagate to the other, and nothing catches the drift until a real `tofu apply` actually tries to use the new name.

**Fix:** added `standard.xlarge` to both files' own `stringvalidator.OneOf(...)` lists; provider rebuilt and reinstalled into the dev-overrides plugin path.

**Verified by:** `tofu validate`/`go build`/`go vet` all clean; a real `tofu apply` using `standard.xlarge` for `worker_flavor` succeeded. (The same structural gap recurred once more for `standard.2xlarge` — see F-121, which also notes the real single-source-of-truth fix this doesn't yet have.)

### F-100 — A rejected build still wiped local Terraform state against a real, already-running deployment, and the recovery path (`tofu import`) hit its own separate provider bug

**Where:** `api/tofu_engine.py`'s existing "wipe local `terraform.tfstate` before every build attempt" behavior, combined with the provider's `InstanceResource.ImportState` (`instance.go`).

**Symptom:** found live while testing F-099's fix — a deliberately-bogus-peer build request, meant to be rejected before ever reaching Terraform, instead tripped the engine's own pre-build state wipe against `examples/llm-chat`'s real, already-running deployment. The follow-up `destroy`/`apply` both correctly failed at F-099's own plan-time validation error before touching anything remote (confirmed: both real instances stayed `running` with unchanged `created_at` throughout) — but local state bookkeeping was left empty, out of sync with reality.

**Root cause:** the state wipe happens unconditionally before validation, not after a build is confirmed valid, so a request that was always going to be rejected still destroys local tracking of a real, healthy deployment. Recovering via `tofu import` then hit a second, genuine, previously-undiscovered provider bug: `InstanceResource.ImportState` never set `state.Users` at all, which the framework can't turn into a valid value on its own from a bare Go zero value — every import of a real instance with no extra `users` configured (the common case) failed outright.

**Fix:** added a proper `instanceUsersFromAPI` conversion (`instance.go`) following the same null-preserving convention already used for `security_group_ids`/`tags`.

**Verified by:** all 8 real resources (vpc/subnet/sg/lb/target-group/listener/both instances) re-imported cleanly afterward, and a `tofu plan` confirmed no drift against reality except the pre-existing, structural "`user_data` forces replacement on import" limitation (the API never echoes `user_data` back on `GET`, by design — a standing limitation of this resource, not a regression). No `tofu apply` was run against the live deployment at any point during this investigation; nothing was destroyed for real. The state-wipe-before-validation ordering itself was not restructured this round, only worked around via re-import — a real, still-open follow-up.

### F-101 — F-099's own fix over-corrected: reconstructing `users` on every provider call collapsed a real, planned empty list into `null`, breaking every instance in the repo

**Where:** the provider's shared `instanceMapToState()` (`instance.go`), called from every `Create`/`Update`/`Read`/`Import` path.

**Symptom:** a real live-fire regression, found by the user's own real `llm-chat` rebuild immediately after F-099/F-100 shipped — the rebuild failed on `module.workers`' instance with "Provider produced inconsistent result after apply", on literally every real instance in this repo that used the default `users = []` (i.e. `modules/instance-group`'s own default).

**Root cause:** F-100's `instanceUsersFromAPI` conversion, called from the *shared* `instanceMapToState()`, ran on every single call, not just `Import`. `users` is Optional but not Computed, so Terraform requires Create/Update's final state to equal the *planned* value exactly — but the API's `GET` always returns `"users": []` whether the config set `[]` explicitly or left it unconfigured, and reconstructing it from that response inside the shared function occasionally collapsed a real, planned `[]` into `null`.

**Fix:** moved the reconstruction out of the shared function entirely. `instanceMapToState()` no longer touches `state.Users` at all — Create/Update/Read already carry the correct value from plan/prior state, the same way `user_data` already implicitly relies on. `ImportState` — the only caller with nothing to inherit from — now calls `instanceUsersFromAPI()` itself, directly, after `instanceMapToState()` returns.

**Verified by:** live. This project's own auto-cleanup-on-failure feature correctly destroyed the partial resources the failed rebuild had already created (vpc/subnet/sg/lb/worker instance) before the error, confirmed via the API afterward — nothing orphaned. A real `llm-chat` build (real paired-peer worker + coordinator + lb + sg + vpc) then completed successfully end to end with the fixed provider.
...
### F-114 — number reserved, content unrecoverable

No `### F-114` section was ever written, and unlike F-099/F-100/F-101 (recovered from Document History prose below and given proper sections above), no trace of its substance exists anywhere in this file's own history (`git log -p -S"F-114"` across the whole repo returns nothing beyond the same one flagging sentence that first surfaced this gap). Documented explicitly, rather than silently skipping the number or inventing content to fill it, so Sentinel's KB and anyone reading this log know the gap is real and known, not an accident. If the original finding is ever recalled or reconstructed from another source, it belongs here.

### F-115 — Stage 9's own exit-code capture misdiagnosed every command, success included, as a failure; a follow-up fix for it then broke the regex entirely

**Where:** `examples/llm-chat/files/verify_proxy.py` (and its `ansible/` mirror) — `_captureAndDiagnose()`'s own sentinel-matching logic, part of Stage 9 (tying "Run in Terminal" back to the model for automatic fault diagnosis).

**Symptom:** live-testing the new feature end to end, a genuinely *successful* command (`lsblk`, exit 0) triggered an automatic "this failed, let me fix it" turn anyway — caught not by the intended test (a deliberately failing command, which correctly worked) but by noticing a stray in-flight model generation the test hadn't triggered on purpose. A first fix attempt (requiring digits after the sentinel's colon via `new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + ':(\\d+)')`) then broke the page **entirely** — a real `SyntaxError` on the live, deployed script, caught immediately by the same live-testing discipline rather than assumed fixed.

**Root cause:** two independent bugs, found and fixed in sequence. First: the command sent to the terminal is `bash <<'DELIM' ... DELIM\necho "MARKER:$?"`. The shell's own local echo of that *typed* input (`student@sandbox:~$ echo "MARKER:$?"`, shown back before it ever runs) also contains the literal text `MARKER:`, just followed by the literal characters `$?"` rather than a digit. The original plain `indexOf(marker + ':')` + `slice()` matched that echoed line first (it appears earlier in the buffer than the real result line), parsed `NaN` from `$?"`, and `NaN !== 0` is always `true` in JavaScript — so *every* command, success included, read as a failure. Second: the fix (require real digits via a regex, escaping `marker` defensively first "just in case") had its own backslash-escaping bug in the very same non-raw-Python-string context this file has hit repeatedly (F-105, F-112) — confirmed by directly importing the module and reading `SANDBOX_PAGE_HTML`'s own real runtime value (the only fully reliable way to check, given a `bash -e`/heredoc-based verification attempt earlier in the same investigation gave misleading results from yet another, uncontrolled layer of shell escaping). The escaped character class never actually closed, breaking the whole inline `<script>` tag.

**Fix:** the sentinel match now requires real digits immediately after the marker's own colon (`new RegExp(marker + ':(\\d+)')`), correctly skipping the echoed-command line while matching the real result line. The defensive escaping was removed entirely rather than re-fixed — `marker`'s own value (`CLOUDCORE_RC_` + `Math.random().toString(36)`) is always plain alphanumeric/underscore by construction, never a single regex-special character, so escaping it was unnecessary complexity in the first place and the exact thing that caused the second bug.

**Verified by:** live, at two levels. The exact deployed JS was independently confirmed syntactically valid by importing the real Python module and reading `SANDBOX_PAGE_HTML`'s actual runtime string (not a hand-traced or shell-escaped reconstruction) through `node --check`. The exact command-construction and matching logic was then validated against the real, running terminal service via a direct raw-WebSocket test (bypassing any browser) covering three cases — `true` (rc=0), `false` (rc=1), `exit 42` (rc=42) — all resolved correctly within roughly 0.1s of the real server's own response. Separately, live end-to-end testing surfaced a real, if unrelated, infrastructure fact worth recording: `llama-server`'s own 14B model was consuming 87% of the coordinator's 3.8GB RAM (a legitimate, expected footprint, not a leak) after this session's own sustained heavy use, at times severely degrading real-time responsiveness; restarting it (standing permission for lab-infra resets between phases) restored healthy headroom. A further, separately-diagnosed oddity — headless Chrome under CDP automation showing consistently delayed `xterm.js` buffer updates (seconds behind the real, sub-second server relay, confirmed by the same raw-WebSocket test) even on a freshly restarted, idle coordinator — was concluded to be a headless-browser rendering-pipeline artifact specific to CDP-driven test automation (xterm.js's own buffer sync is plausibly tied to a rendering loop that a display-less, `--disable-gpu` headless instance does not drive promptly), not a product defect, and not something a real student's own visible browser tab would experience.

### F-116 — A live-load-adjusted vCPU capacity check made a deliberately-tight flavor nearly impossible to ever pass, even on an idle host

**Where:** `api/capacity_gate.py`'s new `affords()` function, built as part of automatic best-host placement for `examples/llm-chat`'s coordinator (picking the best (host, flavor) pair across this host and every approved peer, rather than always defaulting to local).

**Symptom:** caught before ever shipping, via this project's own live-verification discipline — testing `affords()` against llwyn-y-groes's real stats (8 cores, `load_pct_1m` ~1.6%, i.e. essentially idle) with the new `standard.2xlarge` flavor (6 vCPU, deliberately sized to `cores - HOST_RESERVED_CORES` on that host) incorrectly returned `false`.

**Root cause:** the first implementation applied the live CPU-load fraction as a multiplicative discount on top of an already-maximal static reservation: `free_cores = cores * (1 - load_pct_1m/100) - HOST_RESERVED_CORES`. Since `standard.2xlarge`'s own vCPU count was deliberately chosen to exactly saturate `cores - HOST_RESERVED_CORES` when idle (per a direct correction — "we should leave 2 cores free for o/s overhead etc" — while sizing that same flavor, see `[[feedback-vm-core-reservation]]`), *any* nonzero background load at all, however small, pushed the result just under the required 6, making the check fail almost regardless of how idle the host genuinely was (`8 * (1 - 1.6/100) - 2 = 5.872 < 6`). A vCPU allocation is also not really the same kind of resource claim as RAM/disk — hosts already time-share vCPUs across VMs via the hypervisor's own scheduler, so "how busy is the CPU right now" isn't the right signal for "is there room for one more static allocation," unlike RAM's genuinely-shrinking `available_mb`.

**Fix:** made the vCPU check purely static — `free_cores = cores - HOST_RESERVED_CORES`, `free_cores >= flavor_vcpus` — dropping the load-percentage multiplication from this specific gate entirely. Live CPU load remains meaningful only as the pre-existing tie-break signal among hosts that already afford the same flavor tier (`best_fit()`'s own ranking), not as a second, compounding hard gate.

**Verified by:** live, before ever presenting results — isolated `python3 -c` arithmetic checks of both the old and new formula against llwyn-y-groes's real numbers, then a real restart of the local CloudCore API and a live `GET /v1/peers/recommend-placement?flavor_candidates=standard.2xlarge,standard.xlarge,standard.large` request, confirming llwyn-y-groes now correctly reports `best_flavor: "standard.2xlarge"` while the local 4-core coordinator host correctly reports `best_flavor: null` (can't afford even `standard.large`'s 4 vCPU once `HOST_RESERVED_CORES` is subtracted). Re-confirmed identically after the frontend (`ui/src/js/20-tofu-manager.js`) was wired up to consume this endpoint.

### F-117 — `extract_template_vars()`'s regex-based `.tf` parser truncated any unquoted default containing internal whitespace, silently corrupting `llm-chat`'s own `preview_ports` list default

**Where:** `api/tofu_engine.py`'s `extract_template_vars()` — the fallback branch of its `default = ...` extraction (used to expose a template's variable schema, including default values, to both the Dashboard's OpenTofu Build Manager and the actual build submission's own fallback-to-default logic).

**Symptom:** a real, fresh `llm-chat` build (the first full rebuild against `preview_ports` since that variable was added for the Terminal/preview-ports feature) failed at `tofu apply` with `Error: Missing expression ... for <value for var.preview_ports> ... Expected the start of an expression, but found the end of the file.` — reported directly by the user retrying the build in a newly rebuilt lab environment.

**Root cause:** the old fallback regex, `default\s*=\s*(\S+)`, stops at the first whitespace character. `preview_ports`'s own real default, idiomatically formatted with a space after each comma (`default = [41001, 41002, 41003, 41004]`), was captured as only `"[41001,"` — a truncated, syntactically invalid, unterminated list literal. That corrupted string then flowed straight through: the Dashboard rendered it as the field's pre-filled value, and — since it was non-empty — `tfSubmitBuild()`'s existing "only include a var if its field has a value" logic (unrelated to today's placement work, and not itself a bug) happily included it in the submitted vars, which `api/tofu_engine.py`'s `_build_env()` then set as `TF_VAR_preview_ports='[41001,'` verbatim (a plain string passthrough, not JSON-encoded, since every UI-submitted value arrives as a JS string). OpenTofu's own HCL parser reads that as a value with nothing after the trailing comma — hence "found the end of the file", reproduced exactly via a standalone `tofu plan` test with that literal string as the env var. A repo-wide scan confirmed `preview_ports` is the only variable, across every example template, that both lacks quotes and contains internal whitespace in its default — so no other template was silently affected, but any future unquoted list/object default with realistic formatting would have hit the same truncation.

**Fix:** replaced the two-regex (quoted-string-or-bare-token) extraction with a new `_extract_default_value()` that handles three real shapes in order — a quoted string (unchanged), a bracketed list/tuple or braced object/map extracted via real bracket-depth tracking (the same technique `_extract_balanced_block()` already uses for the surrounding variable block, so internal commas/whitespace/nesting can't truncate it), and finally the original single-bare-token fallback (numbers, bools, bare identifiers — every other variable in this repo). `api/build_engine.py`'s own, separate `extract_template_vars()` (for Ansible playbook templates) was checked and confirmed unaffected — it parses real YAML via `yaml.safe_load()`, not regex, so it was never exposed to this class of bug.

**Verified by:** live, at every layer. Isolated Python: re-ran `tofu_engine.extract_template_vars('llm-chat')` directly, confirming `preview_ports` now returns the full, correct `'[41001, 41002, 41003, 41004]'` string (previously `'[41001,'`), with every other variable's default unchanged. Isolated OpenTofu: a standalone `tofu plan` with the corrected value as `TF_VAR_preview_ports` parsed and planned cleanly (previously reproduced the exact reported error with the truncated value). Live API: the real CloudCore API server restarted with the fix, and `GET /v1/tofu/templates/llm-chat/vars` confirmed returning the corrected default over HTTP; the same restart re-confirmed `GET /v1/peers/recommend-placement?flavor_candidates=...` (F-116's own fix) was unaffected by the restart. A repo-wide scan of every example's `variables.tf` confirmed no other template variable was silently corrupted by the old regex.

### F-118 — The first automatic-continuation prompt made the model restart its whole answer from the beginning instead of truly resuming, risking silently duplicated content

**Where:** `examples/llm-chat/files/verify_proxy.py` (and its `ansible/` mirror) — `_continuation_user_turn()`, part of the new context-length auto-continuation feature (see the v0.78 Document History entry).

**Symptom:** caught before ever shipping — testing the continuation mechanism against a response deliberately cut off very early (a small `max_tokens`, to exercise the `finish_reason: "length"` path quickly rather than waiting out a real ~4096-token fill), the model's continuation round didn't continue at all: it just restarted its whole answer from the beginning ("Certainly! A TCP handshake is..." repeated verbatim, several times over, once per continuation round), which would show up as real, silently duplicated content in a genuine long answer.

**Root cause:** the first version of the continuation prompt was purely instructional — "Continue your previous answer exactly where it left off. Do not repeat anything already said, do not restart..." — trusting the model to remember, from a separate fresh completion request, exactly where an earlier, now-history-only assistant turn had been cut off. Under aggressive truncation this instruction alone wasn't reliable; the model treated the new user turn as an ordinary fresh question and answered from scratch instead.

**Fix:** `_continuation_user_turn()` now quotes the literal last `_CONTINUATION_TAIL_CHARS` (300) characters of what the model already wrote back at it, framed as "here is the exact end of what you already wrote... continue writing from that exact point onward" — a well-established stronger technique than an abstract instruction alone, since it gives the model concrete text to pattern-match against rather than trusting it to recall where a separate completion left off.

**Verified by:** live, before and after the fix, using the same artificially aggressive small-`max_tokens` test (fast and deterministic vs. waiting out a real context fill) — the first version reliably reproduced the full-restart duplication across multiple rounds; the tail-grounded version produced clean, non-duplicating continuation across 4 total rounds (1 initial + 3 continuations) even under a brutal 15-tokens-per-round cutoff, with only one cosmetic single-digit artifact (a numbered list item's leading digit duplicated once, from the model treating a mid-number cut ambiguously) — a non-issue at the much larger truncation points a real 4096-token context ceiling actually produces.

### F-119 — `llama-server` can silently deadlock when RPC offloading is in play, wedging the entire coordinator for every future request; a self-healing watchdog was built to detect and recover from it

**Where:** `examples/llm-chat/files/verify_proxy.py` (and its `ansible/` mirror) — discovered while live-testing F-118's own continuation feature against a real, long, un-forced answer; `llama-server.service` itself (the coordinator's own upstream, `--rpc <worker>:50052 -ngl 34`) is the actual site of the bug, not code in this repository.

**Symptom:** a real, un-forced Linux Help question generated normally for several minutes (steady ~1.3 tok/s, confirmed via `llama-server`'s own `print_timing` log lines) then went completely silent — zero further tokens, no `finish_reason` ever arriving, indefinitely. `GET /slots` showed the task permanently `is_processing: true` at a frozen `n_decoded`. Confirmed NOT a one-off: a freshly restarted `llama-server`, given a brand-new small prompt, deadlocked again immediately — and continued recurring on the large majority of real (non-forced) requests sent during this session's own live verification, not as a rare edge case.

**Root cause:** a genuine internal deadlock in `llama-server`'s own thread scheduling when RPC offloading is active — confirmed conclusively via `/proc/<pid>/task/*/stack` on both ends of the RPC connection. The worker's own `ggml-rpc-server` is completely healthy: its main thread sits in `recvfrom()`/`tcp_recvmsg` (idle, waiting for the next request), its compute threads sit in `futex_wait` (idle, waiting for work). Every `llama-server` thread except its plain HTTP `accept()` listener sits in `futex_wait` too — genuinely stuck on an internal lock, not doing real compute or I/O. This is a real bug in `llama-server`/`ggml-rpc` itself (documented upstream as an experimental, not fully hardened backend), not anything in this deployment's own Python/Terraform/Ansible code — restarting `llama-server.service` was the only thing that ever cleared it; nothing short of that did. Two further, real sub-bugs were found and fixed while building the watchdog itself (before it ever shipped): first, `llama-server`'s own SSE keep-alive pings keep the raw socket "alive" even while fully deadlocked, meaning a naive connection-level read timeout never catches this at all — the watchdog has to track time since the last *real content byte*, not just any byte. Second, `http.client`'s own buffered reader raises a plain `OSError("cannot read from timed out object")` on the read immediately *after* the first `socket.timeout` on a chunked-transfer response (not another `socket.timeout`), which the first implementation didn't catch, crashing the request-handling thread outright — caught live via a real journal traceback during testing, fixed by catching `(socket.timeout, OSError)` together with a brief sleep to avoid busy-looping if the connection really is broken.

**Fix:** new `GENERATION_STALL_TIMEOUT_S` (default 120s, `generation_stall_timeout_seconds` threaded through Terraform/Ansible) tracks time since the last real content delta across `_relay_one_stream`'s read loop (shared by the initial ask and every continuation round, so this protection covers both paths for free); once exceeded, the student is told honestly ("the model backend appears to have stalled... being automatically restarted") and a background thread fires `systemctl restart llama-server.service` (`verify-proxy.service` runs as root, no sudo prompt needed) — a `_LLAMA_RESTART_COOLDOWN_S` (30s) guards against several concurrent students all triggering redundant restarts for the same underlying stall. Confirmed live, separately: `verify-proxy.service`'s own systemd unit has a pre-existing `Requires=llama-server.service`, so this restart also briefly restarts `verify-proxy.service` itself (systemd's own dependency propagation) — an acceptable, low-cost side effect (a few seconds, no model reload) documented plainly in the code rather than worked around, since nothing depends on the restarting process surviving past that point anyway.

**Verified by:** live, repeatedly and autonomously — the watchdog fired correctly four separate times during this session's own real-usage testing (not staged), each time waiting the full, correct 120s before acting (confirmed via `llama-server`'s own journal timestamps matching to the second), writing the honest student-facing message, and successfully restoring `llama-server.service` to a healthy, responding state each time (confirmed via `GET /health` and `GET /slots` after each recovery). Given how frequently the underlying deadlock recurred under this session's own repeated real-request testing, this is flagged plainly as a live, active reliability concern, not a purely theoretical safety net — worth the user's own attention regarding whether RPC offloading should keep running in its current form.

### F-120 — `worker_flavor` was sized to a 7B model's RAM needs and never revisited when the default model stepped up to 14B, leaving the RPC worker with almost no memory headroom

**Where:** `examples/llm-chat/variables.tf` (and its `ansible/` mirror, `14-llm-chat.yml`) — `worker_flavor`'s own default.

**Symptom:** a real overnight incident — a student left a question running, and it never returned (no error, no answer, nothing) for 8+ hours. Investigation of the coordinator's own journal found `llama-server` had genuinely crashed (not the usual silent F-119 deadlock): `ggml-rpc.cpp:566: Remote RPC server crashed or returned malformed response`, followed by a real `ggml_abort()` backtrace and `Main process exited, code=killed, status=6/ABRT`. systemd's own `Restart=on-failure` correctly recovered `llama-server.service` within about a minute, but that also cascades to restart `verify-proxy.service` (the same pre-existing `Requires=llama-server.service` dependency documented in F-119) — abruptly, mid-request, before it could report anything to the student's browser at all. That specific gap (a hard crash, not a silent stall, leaves the client with literally nothing) is real and distinct from what the F-119 watchdog covers, and is tracked as a known limitation rather than fixed this round.

**Root cause:** checking the RPC worker's own state at the time of the crash (`free -h`, run directly on the worker VM) showed only ~1.7GB free out of 7.8GB total, with the `ggml-rpc-server` process itself already using ~5GB just to hold its own share (34 offloaded layers) of the 14B model plus its KV cache slice. `worker_flavor`'s own comment history explains why: it was sized to `standard.xlarge` (8GB) by matching the paired peer's real 8-core CPU count when `worker_flavor` was first split out from `coordinator_flavor` — but the model default was stepped up from 7B to 14B (roughly doubling the real memory footprint) in a *later*, separate change, and the worker's own RAM sizing was never revisited after that jump. A worker running this close to its own ceiling is a highly plausible contributor to real instability beyond just F-119's own upstream threading bug — not confirmed as the sole cause of every stall/crash this session, but a real, self-inflicted gap independent of it. `GET /v1/peers/<id>/stats` confirmed live that the underlying host (llwyn-y-groes) genuinely has 17GB+ available right now, so this was never a host-capacity problem, only a stale flavor default.

**Fix:** `worker_flavor`'s default raised from `standard.xlarge` (8GB) to `standard.2xlarge` (16GB) in both the Terraform and Ansible copies, restoring a real, comfortable margin. Chasing this surfaced a second, separate, live-blocking bug (F-121, below) that would have silently broken this exact fix.

**Verified by:** live — `GET /v1/peers/<peer_id>/stats` against the real running local CloudCore API confirmed the host's own real available RAM before committing to the bigger flavor size. Not yet verified end-to-end against a real rebuilt worker (would need a fresh `llm-chat` build) — the corrected default is confirmed accepted at plan time (see F-121's own verification), but a worker actually running at `standard.2xlarge` under real load hasn't been observed yet.

### F-121 — The OpenTofu provider's flavor validator never learned about `standard.2xlarge`, the same class of gap F-099 already fixed once for `standard.xlarge`

**Where:** `provider/internal/resources/instance.go` and `provider/internal/resources/nfs_server.go` — both files' own hardcoded `stringvalidator.OneOf(...)` flavor lists.

**Symptom:** caught while fixing F-120 above, before it could bite for real — `api/compute.py`'s `FLAVORS` dict already knew about `standard.2xlarge` (added earlier this session for the coordinator best-fit placement work), but the OpenTofu provider's own attribute-level validators for both `cloudcore_instance` and `cloudcore_nfs_server` still hard-coded only the original five flavor names. Setting `worker_flavor`'s default to `standard.2xlarge` (F-120) would have failed at `tofu plan`/`apply` time with a provider validation error the moment a real build actually tried to use it — exactly the same failure mode F-099 already documented once for `standard.xlarge` being added to `api/compute.py` without the provider's own validator being updated alongside it. Confirmed via `grep` that `standard.2xlarge` had, in fact, never actually been exercised through a real `tofu apply` this session — every earlier test of it stayed at the API-response level (`recommend-placement`), or the coordinator ended up staying local (worker already claimed the only peer) whenever a real build ran, so this gap stayed latent until now.

**Root cause:** the two files' own flavor validators are separate, hand-maintained string lists, not derived from `api/compute.py`'s own `FLAVORS` dict — adding a new flavor there doesn't automatically propagate to the provider's own Go source, and nothing catches the mismatch until a real `tofu apply` actually tries to use the new name. Same structural gap as F-099, recurring because the fix for F-099 corrected the immediate symptom (added `standard.xlarge` to both lists) without addressing why the two representations of "the known flavors" can drift apart in the first place — noted here, not restructured this round.

**Fix:** added `standard.2xlarge` to both files' own `stringvalidator.OneOf(...)` lists, matching F-099's own precedent exactly. A single-source-of-truth fix (generating the provider's own validator list from `api/compute.py`'s `FLAVORS` dict, or the reverse) is a real, worthwhile follow-up given this is now the second time the same gap has recurred, but out of scope for this round.

**Verified by:** live — rebuilt the provider binary and reinstalled it into the dev-overrides plugin path; `tofu validate` against `examples/llm-chat` (whose `worker_flavor` default is now `standard.2xlarge`) passed cleanly with no validator error; `go build ./...` and `go vet ./...` against the whole provider module both clean.

### F-122 — `_do_handle_ask`'s own initial connection to `llama-server` stayed open through every continuation round, self-inflicting the exact concurrent-connection condition F-119's own #28908 starvation bug is triggered by

**Where:** `examples/llm-chat/files/verify_proxy.py` (and its `ansible/` mirror) — `_do_handle_ask()`'s own `conn.close()` call, and `_relay_and_verify_stream()`'s signature.

**Symptom:** a student reported a fresh question producing literal SSE protocol text (`data: [DONE]`) as the visible answer, then, on retry, a genuine long stall — both consistent with a request colliding with something else mid-flight. Investigating why F-119's stalls recurred so frequently even with the F-119 busy-gate deployed (which only prevents two *different* students colliding) raised the question of whether something *within* a single student's own request could still trigger the same starvation condition.

**Root cause:** `_do_handle_ask()` opens the initial connection to `llama-server` (`conn`), passes its response into `_relay_and_verify_stream()`, and only closed `conn` *after* that whole function returned. But `_relay_and_verify_stream()` internally may run several F-118 continuation rounds first, each opening its *own* separate connection to `llama-server` (correctly closed right after its own read completes) — meaning the original, already-fully-consumed round-1 connection stayed open the entire time every later round's connection was also open. Two connections from the same client open at once toward the same backend is exactly the condition F-119's own upstream issue (ggml-org/llama.cpp#28908) is triggered by — self-inflicted by the continuation feature itself, not just an unlucky collision with another student. Not confirmed as the sole explanation for the specific `data: [DONE]` text artifact reported live (that mechanism remains unexplained), but a real, clear resource-leak bug regardless of whether it fully explains that one symptom.

**Fix:** `_relay_and_verify_stream()` now accepts the caller's own `conn` and closes it immediately after its own `_relay_one_stream()` call returns — i.e., as soon as that connection's response is fully read, before any continuation round opens a new one — matching exactly how each continuation round already closes its own connection right after its own read finishes. The caller's own trailing `conn.close()` is left in place as a harmless, idempotent safety net.

**Verified by:** both the real and `ansible/` mirror copies compile cleanly; deployed live to the running coordinator, confirmed healthy (`GET /health`) immediately after redeploy. Not yet confirmed to actually reduce real-world stall frequency under live student use — that needs more real usage to observe, not something provable in isolation.

### F-123 — An upstream connection accepted then closed with zero bytes (a real transition-window race) was silently reported as a content-free "success", producing the exact bare `data: [DONE]` artifact a student saw live

**Where:** `examples/llm-chat/files/verify_proxy.py` (and its `ansible/` mirror) — `_relay_and_verify_stream()`'s own handling of `_relay_one_stream()`'s return value.

**Symptom:** a student, in a brand-new private browser tab (ruling out stale history), asked a genuinely fresh question and saw literal SSE protocol text (`data: [DONE]`) rendered as the visible answer — no content, no error, nothing to explain what happened. Reproduced live and directly via a controlled A/B test: firing one request through the LB (`127.0.0.1:8620`, HAProxy) and one directly to the coordinator at the exact same moment, timed to land during a real backend transition — HAProxy correctly detected no backend was available and returned a clean `503 Service Unavailable`; the *direct* request instead got accepted, connected to `llama-server`, and got back a response whose body closed with zero bytes and no `finish_reason` at all.

**Root cause:** `_relay_one_stream()`'s own default return for this case — `stop_reason=None`, `finish_reason=None`, empty accumulated text — is indistinguishable, to the rest of the function, from an entirely ordinary (if unusually terse) successful completion: the continuation loop correctly never enters (finish_reason isn't "length"), and none of the existing `stop_reason` branches (`disconnected`/`interrupted`/`stalled`) fire, so execution falls straight through to the final `[DONE]` write with nothing ever told to the student. This is a real, narrow timing window — a TCP connect to `llama-server` can succeed and receive real HTTP response headers in the split second the process is already tearing its own response down (mid-restart-cascade, per F-119/F-120's own `Requires=llama-server.service` chain) — genuinely distinct from every other failure mode already handled (a stall has real elapsed time and a task in flight; a disconnect is the *client's* pipe breaking, not the upstream's).

**Fix:** `_relay_and_verify_stream()` now explicitly detects this exact shape (`stop_reason is None and finish_reason is None and not text`) right after the initial `_relay_one_stream()` call and sets a new, distinct `stop_reason = "empty_response"`, with its own honest student-facing message ("the model backend closed the connection unexpectedly... please try again") — genuinely different advice from the "stalled" case (this is a momentary transition window, not a hung process needing a restart, so a plain retry is the right guidance, not another `systemctl restart`).

**Verified by:** live, deliberately reproduced twice via a targeted test — polling the coordinator's own `/health` to detect the exact instant a real stall-triggered restart began, then firing a burst of concurrent requests right at that transition. Two of the five landed in the exact race window and correctly received the new honest message instead of the old bare `data: [DONE]`; the other three correctly hit the F-119 busy-gate's own rejection message, confirming both mechanisms work together correctly. Both the real and `ansible/` mirror copies compile cleanly.

### F-124 — Neither HAProxy nor `verify_proxy.py` recorded anything for a real failure to be understood after the fact, so every incident this session required live, in-the-moment SSH investigation

**Where:** `api/lb.py` (every generated HAProxy config, via `_write_config()`) and `examples/llm-chat/files/verify_proxy.py` (a new `_log_ask_outcome()`).

**Symptom:** not a bug in the traditional sense — a direct observation, made explicit by the user, that every F-119/F-120/F-122/F-123-class incident this session could only be understood by live SSH investigation *while it was still happening* (checking `/slots`, `/proc/<pid>/task/*/stack`, journal timestamps in real time). Nothing was ever recorded anywhere for a later look, including HAProxy itself, which had no `log` directive in its generated config at all.

**Root cause:** `_write_config()` never emitted a `log` directive (confirmed live: `/var/log/haproxy.log` existed, group-readable, completely empty — the file Ubuntu's own `haproxy` package's `/etc/rsyslog.d/49-haproxy.conf` already routes `haproxy`-programname syslog messages into, entirely unused). `verify_proxy.py` separately had `_record_llm_stats()`, but that function is only ever called on a genuine completion with real `timings` data — every *other* outcome (stalled, disconnected, interrupted, upstream-unreachable, empty-response, hit-continuation-ceiling) left no trace at all.

**Fix:** `_write_config()` now emits `log /dev/log local0` in `global` plus `log global` in `defaults`, and `option httplog`/`option tcplog` (mode-appropriate) for full per-request access-log detail (timers, backend server used, status, termination flags) — no new rsyslog config needed, the existing system-wide HAProxy rule already routes it to `/var/log/haproxy.log`. New `_log_ask_outcome()` emits one structured JSON line (`ASK_OUTCOME {...}`, captured by `verify-proxy.service`'s own journal like every other diagnostic line in this file) per ask request covering every real outcome — `success`, `stalled`, `disconnected`, `interrupted`, `upstream_unreachable`, `length_exhausted`, `empty_response` — with duration, question/answer length, continuation rounds used, and token throughput when available.

**Verified by:** live. HAProxy: confirmed the running process picked up the new config via a real graceful reload (`-sf`, new PID observed), generated a real request, and confirmed both a genuine access-log line (full timing/status/termination detail) and an unrelated pre-existing backend-DOWN health-check message (an orphaned legacy backend on port 80 nothing routes to, now visible only because logging is on, not introduced by this change) both appeared correctly in `/var/log/haproxy.log`. `verify_proxy.py`: confirmed a real `"stalled"` outcome logged with `duration_s: 120.12` (matching `GENERATION_STALL_TIMEOUT_S` to the hundredth of a second) and the correct question length, across two separate real stall occurrences on two different process PIDs (i.e. surviving a service restart in between, confirming it isn't tied to any one process instance).

### F-125 — A deadlocked `llama-server` doesn't even respond to `SIGTERM`, so a "graceful" restart silently cost 90 extra seconds waiting out systemd's own default stop timeout before it escalated to `SIGKILL`

**Where:** `examples/llm-chat/files/coordinator-cloud-init.yaml.tftpl` (and its `ansible/` mirror) — `llama-server.service`'s own `[Service]` block, previously missing `TimeoutStopSec`.

**Symptom:** a student reported the same question appearing twice in the transcript, the first attempt stuck permanently on "Thinking…" with no resolution ever arriving, the second correctly receiving the honest "stalled, restarting" message but only after a noticeably long wait. The real coordinator journal for this exact incident showed the F-119 watchdog fired correctly and on schedule (`ASK_OUTCOME "stalled"` at the expected 120s), but `systemctl restart llama-server.service` then took a further 90 seconds before anything happened: `State 'stop-sigterm' timed out. Killing.`, followed by a hard `SIGKILL` of every one of `llama-server`'s own child processes. Only after that did the usual ~90s model reload even begin — the second student attempt landed squarely inside this much longer than usual outage.

**Root cause:** `llama-server.service` had no `TimeoutStopSec` set, so systemd used its own platform default (90s) before escalating a stuck `SIGTERM` to `SIGKILL`. A process deadlocked the way F-119 describes (every thread genuinely parked in `futex_wait`, not doing real work) has no reason to ever respond to a plain terminate signal on its own — the 90-second wait was pure, unavoidable dead time, not a safety margin protecting anything: `llama-server` is a stateless inference server with no transactions, writes, or in-flight state worth a graceful shutdown.

**Fix:** added `TimeoutStopSec=5` to the unit in both the Terraform and Ansible copies — a real deadlock now escalates to `SIGKILL` in 5 seconds instead of 90, cutting total recovery time by roughly that same 85 seconds, for both the F-119 watchdog's own deliberate restarts and any future crash systemd's own `Restart=on-failure` handles independently. The specific "first bubble stuck on Thinking… forever, no message ever arrives" half of the symptom remains only partially explained — most plausibly the coordinator process being killed abruptly enough that an already-buffered-and-flushed SSE write never actually made it out over the socket before the process's own file descriptors were torn down — flagged honestly as a real, if narrow and hard-to-close-completely, remaining gap rather than claimed fully fixed.

**Verified by:** live — applied directly to the running coordinator's own unit file (`TimeoutStopSec=5` added, `systemctl daemon-reload` confirmed via `systemctl show -p TimeoutStopUSec` reporting `5s`) without disturbing the already-healthy running process. Not yet observed live against a fresh real deadlock recurring on this exact instance (the underlying F-119 bug is intermittent) — the fix itself is a plain, low-risk systemd unit setting, not something requiring a forced reproduction to trust.

### F-126 — The coordinator's `-ngl`/`--rpc` flags weren't actually safe with zero workers, even though `worker_peers` being empty was already meant to be a supported, ordinary state

**Where:** `examples/llm-chat/locals.tf` and `files/coordinator-cloud-init.yaml.tftpl` (and their `ansible/` mirrors) — the coordinator's own `llama-server.service` `ExecStart` line.

**Symptom:** caught before it could bite for real, while designing a deliberate "coordinator alone, no RPC workers" deployment mode (per direct request: keep `worker_peers`/RPC offloading fully available for later hardware, but let the coordinator run the whole model itself, on whichever host — including a peer — can actually carry it, right now). With `worker_peers = []`, `local.rpc_servers` (the `--rpc` argument's own value) evaluates to an empty string — the old, unconditional `-ngl ${rpc_offload_layers} --rpc ${rpc_servers} -t ${threads}` would have collapsed to `-ngl 37 --rpc  -t 4` (the empty substitution merging `--rpc`'s own whitespace with `-t`'s), which a command-line parser reads as `--rpc`'s value literally being the string `-t`, not two separate flags — `4` would then be `llama-server`'s own next positional/unrecognized argument, breaking startup outright. `worker_count = length(var.worker_peers)` already made zero workers a structurally valid Terraform state (module.workers simply creates zero instances), but nothing downstream had actually been exercised with it — this codepath was written when a worker was always assumed to exist.

**Root cause:** the ExecStart line's `-ngl`/`--rpc` arguments were always included unconditionally, with no equivalent of the flavor of "only emit this when it's actually meaningful" handling the rest of this template already applies elsewhere (e.g. `coordinator_placement_overrides`'s own empty-map-when-unset pattern).

**Fix:** new `local.llama_rpc_flags` (`locals.tf`) — `"-ngl <n> --rpc <servers> "` (note the trailing space) when `worker_count > 0`, or an empty string when it's 0 — replaces the two separate variables in both the `templatefile()` call and the `ExecStart` line itself (`${llama_rpc_flags}-t ${threads}`, no `-ngl`/`--rpc` at all when there are no workers, matching llama.cpp's own "no offload target = run every layer locally" default). Mirrored in the Ansible copy via an equivalent `ternary()`-based fact. A stale, unrelated bug surfaced by this same change: a top-of-file YAML comment in `coordinator-cloud-init.yaml.tftpl` still referenced `${rpc_servers}` directly — since `templatefile()` parses `${...}` interpolations everywhere in the raw file, including inside `#`-prefixed comments, this alone broke `tofu validate` outright ("Invalid function argument") the moment the variable it referenced stopped being passed in; rewritten to describe the now-conditional behavior in plain prose instead.

A second, separate blocker surfaced only when actually submitting a real coordinator-alone build (`worker_peers = []`): a pre-existing `variable "worker_peers"` validation rule in `variables.tf` (`length(var.worker_peers) > 0`, error message "this template's whole purpose is cross-host distributed inference") and a matching `ansible.builtin.fail` fast-fail task in `ansible/examples/14-llm-chat.yml` both still encoded the old assumption that a worker was mandatory — neither is exercised by `tofu validate`/lint, only by a real submit. Fixed by removing both gates (`variables.tf` gains `default = []` in their place) and replacing them with comments recording why an empty `worker_peers` is now a deliberately supported, ordinary state, not an oversight.

**Verified by:** `tofu validate` (clean after fixing the stale comment reference), a direct Python string-substitution check confirming the rendered `ExecStart` line is correct and cleanly spaced in both the with-workers and zero-workers cases, and `yaml.safe_load()`/`jinja2.Environment().parse()` confirming the Ansible mirror's own YAML and Jinja2 syntax are both valid. Also verified against a real `tofu apply` with `worker_peers` genuinely empty (build id `b853e131-8690-4c97-953d-f0ccf344192d`, coordinator instance `fbf0831b-d956-465e-92c4-e6f63fcd9e44` on llwyn-y-groes): destroy of the prior RPC-enabled deployment (17 resources) and a clean rebuild (4 resources, zero worker instances) both succeeded, and the real running `llama-server.service` unit on that instance was confirmed via direct SSH to carry no `-ngl`/`--rpc` flags at all.

### F-127 — The `llama-server` deadlock is not RPC-specific: the same stall recurs at a 100% rate on a coordinator-alone (zero-worker) deployment

**Where:** `examples/llm-chat/files/verify_proxy.py`'s `_relay_one_stream()`/F-119 watchdog path; the underlying deadlock is inside `llama-server` itself (build `b11025`), not this codebase.

**Symptom:** after F-126 shipped a genuine coordinator-alone deployment (`worker_peers = []`, no `--rpc` flag at all, real build `b853e131…` verified with no `-ngl`/`--rpc` in the running unit) on the working theory that dropping RPC would fix the stalls documented in F-118/F-119/F-122 (all attributed to upstream [ggml-org/llama.cpp#28908](https://github.com/ggml-org/llama.cpp/issues/28908), an RPC-worker `accept()`-loop starvation bug triggered by a second concurrent connection), the identical symptom recurred immediately — with genuinely zero RPC connections possible. A live `ASK_OUTCOME` query against Loki for the ~6 hours since the coordinator-alone rebuild (2026-09-23 09:47–15:40 UTC) shows 13 of 14 real requests ending `outcome: "stalled"`, every one landing at 120.05–120.27s (matching `GENERATION_STALL_TIMEOUT_S` to within its own measurement granularity) with `answer_chars: 0` in every single case — never partial content, never a different duration. Two earlier live-catch attempts this same investigation both mistakenly appeared to show "125 characters of real content before stalling" — corrected on inspection: `len()` of this codebase's own hardcoded fallback message is exactly 125, and the parsing script was counting its own injected error text, not model output. The user's own direct `journalctl -u llama-server.service | grep print_timing` check independently confirmed zero real generation ever occurred in at least one of these stalls.

**Root cause:** not yet identified. What this rules out, with real evidence: (1) RPC/`ggml-rpc.cpp`'s own accept-loop starvation (#28908) as the *sole* cause — it requires a second concurrent connection to the same RPC worker, structurally impossible with zero workers configured; (2) a load/resource-contention explanation — these are single, uncontended first requests on an otherwise-idle 16GB/6vCPU dedicated host (`standard.2xlarge` on llwyn-y-groes), not concurrent collisions. What it's consistent with: a deadlock or unbounded wait inside `llama-server`'s own request-handling/generation thread(s) that is independent of the RPC transport entirely — plausibly triggered by something specific to this deployment (this exact model, `Qwen2.5-Coder-14B-Instruct-Q4_K_M`; this `llama.cpp` build, `b11025`; the fixed context size, thread count, or webui-config; or the coordinator's own request/prompt shape) rather than the RPC code path F-118/F-119 originally blamed. The 100% failure rate and near-identical stall duration across every attempt (not a rare race) points toward a deterministic trigger, not a low-probability threading race.

**Fix:** none yet — this is an open, actively-investigated root-cause correction, not a resolved finding. The single most valuable next step is a live `/proc/<pid>/task/*/stack` (or `wchan`) capture of `llama-server` during an in-progress stall, to see exactly which syscall/futex every thread is actually blocked on — this requires real-time SSH access to the llwyn-y-groes-hosted coordinator (`192.168.101.82`), which is not reachable from stourport (confirmed live: stourport's own key gets `Permission denied (publickey)` against that host; llwyn-y-groes-hosted instances only accept llwyn-y-groes's own local key, runnable only from that machine). Byte-level SSE timing was captured for one attempt via `urllib` from stourport (no SSE bytes of any kind — not even a keep-alive ping — arrived before the connection was cut) but this only confirms `llama-server` never entered its own streaming-response code path at all for that request, not why.

**Verified by:** live `ASK_OUTCOME`/Loki data (13/14 stalls, 09:47–15:40 UTC 2026-09-23, zero content in every case), a real `/sandbox/ask-status` poll catching one stall live end-to-end (`busy: true` 15:39:38–15:40:18, cleared exactly when the matching `ASK_OUTCOME` line logged `outcome: stalled, duration_s: 120.2` at 15:40:27), and the user's own independent `print_timing` journal check. Not yet verified: root cause. This finding intentionally supersedes the working assumption in F-126's own motivation text (that dropping RPC would resolve the stalls) — it does not, at least not on its own; the original F-118/F-119 RPC-starvation diagnosis may still be a real, separate bug that was never actually the dominant cause of stalls in this deployment.

### F-128 — Caught the RPC-enabled stall live: a mutual `recv()` deadlock on a single, already-ESTABLISHED connection, not the accept-loop starvation #28908 describes

**Where:** llama.cpp's own `ggml-rpc.cpp` (build `b11025`), both the coordinator's RPC client and the worker's `ggml-rpc-server` — not this codebase.

**Symptom:** with the new `claude-debug` SSH access (F-127's own follow-up) baked into a fresh build (`4794811c-90a7-4035-840d-164c0d2c6791`, coordinator `192.168.100.196` on stourport, worker `192.168.101.29` on llwyn-y-groes, `--rpc 192.168.101.29:50052` genuinely wired in), a real question was fired and polled every 3s via `/sandbox/ask-status`, capturing a full `/proc/<pid>/task/*/stack` snapshot on the coordinator's `llama-server` (pid 2697) at every busy poll. For the first ~57s every one of its 17 threads sat in ordinary idle patterns (`futex_wait` on the GGML thread pool, `accept4` on the HTTP listener, timer/reschedule IPIs) — no request-handling activity visible yet. Starting at 16:17:31.881 UTC and persisting through every subsequent poll up to 16:18:34.420 UTC (63+ seconds, unchanged), thread 2796 sat blocked in `wait_woken → sk_wait_data → tcp_recvmsg → inet_recvmsg → sock_recvmsg → __sys_recvfrom` — a genuine kernel-level block waiting for TCP data that never arrived. At 16:18:38 the connection dropped (the coordinator's `llama-server` was killed/restarted by the F-119 watchdog, recovering quickly per F-125's `TimeoutStopSec=5`), and `ask-status` returned to `busy: false` moments later.

Checked the worker side at the same moment (SSH now reachable directly from stourport via the same `claude-debug` key — F-127's own note that llwyn-y-groes-hosted instances were unreachable from stourport is now **superseded** by this access): `ggml-rpc-server` (pid 1927, 4 threads total — the fixed `-t 4` GGML compute pool plus this one connection-handling thread) was alive, not crashed, with a real `ESTABLISHED` TCP socket on `:50052` (source `10.99.100.1`, the WireGuard peer-mesh tunnel endpoint, not a raw `192.168.100.196`-to-`192.168.101.29` route — confirms cross-host RPC traffic rides the existing peer WireGuard tunnel). Its own connection-handling thread (1927) was **also** blocked in the identical `tcp_recvmsg`/`recvfrom` stack, on the same connection, at the same time. Its 3 GGML compute threads sat idle in `futex_wait`, never invoked — the worker never got far enough to actually compute anything.

**Root cause:** not yet identified at the ggml-rpc.cpp source level, but the observed shape is now precise and different from what F-118/F-119 originally cited. This is **one single, already-`ESTABLISHED` connection** — no second connection exists in this reproduction at all (`worker_count = 1`, and the coordinator only ever opens one RPC client connection) — so upstream [ggml-org/llama.cpp#28908](https://github.com/ggml-org/llama.cpp/issues/28908)'s own literal description (a single-threaded `accept()` loop starving a *second* incoming connection while a first stays open) does not fit this specific reproduction: there was no second connection for an accept loop to starve. What was actually observed is a **mutual `recv()` deadlock** — both ends simultaneously blocked waiting to *receive* more data on the same socket, neither one having sent whatever the other is waiting for. Consistent with an RPC message-framing bug (a length-prefixed send left short, or a message sent out of the order the receiver's state machine expects) rather than literal accept-loop starvation — but this is inference from the observed symptom, not confirmed against the actual `ggml-rpc.cpp` protocol implementation yet. F-118/F-119's original #28908 attribution may have been the wrong specific upstream issue, even though the RPC subsystem is still squarely implicated.

**Fix:** none yet — root cause still open. Next step, now unblocked by real SSH access to both hosts: reproduce again with `strace -f -p <pid>` or a debug build attached to both the coordinator's RPC client thread and the worker's connection thread *before* the stall, to see the exact bytes each side sent/expected immediately before both went quiet — this needs to be armed proactively (attaching `strace` after the fact, as this round did with `/proc/*/stack`, only shows the blocked state, not what was sent/missing right before it). Also worth checking whether this same mutual-recv pattern recurs on a second reproduction, or whether this specific run hit a different, rarer variant.

**Verified by:** live, timestamped `/proc/<pid>/task/*/stack` captures on both the coordinator and the worker during a real, in-progress stall (2026-09-23, ~16:16:35–16:18:38 UTC) — the single most direct evidence gathered in this entire investigation arc, made possible by F-127's own `claude-debug` SSH follow-up. Does not resolve F-127 itself (the RPC-*free* coordinator-alone stall, zero network component possible, a genuinely different symptom) — that root cause remains separately open.

### F-129 — Corrected root cause: this deployment's real throughput (not a deadlock) is what actually causes every stall — prefill alone exceeds the old 120s watchdog

**Where:** the underlying cause is real-world throughput of `llama-server` (build `b11025`) on this hardware/network, with `-ngl 39 --rpc <worker>`; `examples/llm-chat/variables.tf`'s/`ansible/examples/14-llm-chat.yml`'s `generation_stall_timeout_seconds` (previously 120) is the codebase-level artifact that made it visible as a hard failure.

**Symptom:** F-128's own live `/proc`/strace evidence (a coordinator thread and a worker thread both blocked in `tcp_recvmsg` on the same single connection) was real and accurately observed, but its interpretation was wrong. A direct request to `llama-server` on the coordinator (`192.168.100.196`), bypassing `verify_proxy.py`'s watchdog entirely, with the actual production-length Linux Help system message (~2.5KB, matching `linux_system_message`) and a generous client-side timeout, **succeeded** — HTTP 200, a complete, coherent, correct real answer — but took **297.4 seconds**. `llama-server`'s own `timings` object in the response gives the exact breakdown: prompt processing (prefill) of 575 tokens took **142.6s (4.03 tok/s)**, and generating 265 tokens took **154.5s (1.71 tok/s)**. Prefill *alone* already exceeds the old 120-second `GENERATION_STALL_TIMEOUT_S` — meaning every single real student question, past or present, RPC or not, was mathematically guaranteed to be killed by the watchdog before a single token could ever be generated. This directly explains F-127's 13/14 100%-stall rate (RPC-free, idle dedicated host) and F-128's RPC-enabled reproduction identically: both are the same underlying "not fast enough for 120s," not two different deadlock bugs.

**Root cause:** `-ngl 39` splits the model's compute across the coordinator and a peer over RPC (this specific build); prefill/generation throughput of ~4/~1.7 tok/s is consistent with a mix of (a) genuine RPC/network round-trip overhead per layer (23.8MB sent to the worker in the SET_TENSOR/GRAPH_COMPUTE setup phase alone for one request, confirmed via `strace`), (b) the coordinator's own host (stourport) being genuinely CPU-contended (`load average: 1.98, 2.87, 2.87` on 4 real cores, competing directly with the coordinator VM's own vCPUs), and (c) this specific 14B model simply being large for the available compute. A raw network throughput test (20MB in 0.9s, ~22MB/s) rules out link bandwidth as the bottleneck — this is compute-bound, not network-bound. F-118's original diagnosis ("a real bug in llama-server/ggml-rpc... a real deadlock") is corrected here: every `/proc/*/task/*/stack` snapshot that looked like a stuck thread was very likely a snapshot of genuinely slow, in-progress work, not an indefinite hang — confirmed directly this round by watching the same kind of "stuck" thread actually resume and complete successfully once given enough real time.

**Fix:** raised `generation_stall_timeout_seconds` (both the Terraform and Ansible copies) from 120 to 420, with real margin above the measured 297s worst case observed so far. Live-patched and confirmed on the running coordinator first (`GENERATION_STALL_TIMEOUT_SECONDS=420` in `verify-proxy.service`) before making it the template default.

**Verified by:** the direct 297.4s/HTTP 200 request with `llama-server`'s own `timings` object (prompt_per_second: 4.03, predicted_per_second: 1.71) — hard, first-party evidence, not inference. Does **not** fully resolve real end-to-end usability on its own — see F-130.

### F-130 — RESOLVED: `verify_proxy.py`'s own relay never observed content `llama-server` successfully generated, because CPython's `socket.SocketIO` permanently poisons itself after one timeout

**Where:** `examples/llm-chat/files/verify_proxy.py`'s `_relay_one_stream()`/`_open_upstream_completion()` (and its Ansible mirror).

**Symptom:** with `GENERATION_STALL_TIMEOUT_SECONDS` raised to 420, a real end-to-end request through the actual student-facing path (`POST /sandbox/linux-ask`, real SSE streaming) was fired twice. Both times: `llama-server`'s own journal proves the request was fully, successfully processed — a real slot assigned, `n_gen` progressing normally, ending in `slot print_timing`/`stop processing` with a genuine `finish_reason: stop` and a complete answer (one run: 828 total tokens, 318.4s) — yet `verify_proxy.py` logged `ASK_OUTCOME {"outcome": "stalled", "answer_chars": 0}` at ~421s both times, and the student-visible result was the generic "model backend appears to have stalled" fallback message, not the real answer. `llama-server` genuinely finished ~100 seconds *before* `verify_proxy` declared the stall.

**What was ruled out, with direct live evidence, not guesswork:**
- Not a network/RPC availability problem — the same request, sent directly to `llama-server` bypassing `verify_proxy` entirely (same host, same payload, same `stream: true`), completes correctly with real content streaming continuously from ~0.7s after headers all the way to a clean `EOF` at ~269s (114,483 bytes total) — proving `llama-server`'s own SSE flushing works correctly in isolation.
- Not `_open_upstream_completion` failing to connect — both live traces show `conn.getresponse()` returning `HTTP/1.1 200 OK` within tens of milliseconds; the response object is real and valid.
- Not a stuck/crashed handler thread — `_handle_ask`'s own busy-gate (`_llm_busy`, set/cleared in a `try/finally` around `_do_handle_ask`) can only stay `True` for as long as the *original* request-handling thread is still alive and running; since `busy: true` persisted the full ~420s and the *same* thread ID reappeared at the very end to log the `stalled` outcome, that thread was provably alive throughout, not dead.
- Not a stuck systemd/service issue — `llama-server.service` and `verify-proxy.service` were both `active` and otherwise responsive (health checks succeeding normally on separate connections) throughout.

**What remains genuinely unresolved:** two independent live `strace -f` traces of `verify_proxy.py`'s own process during a real, confirmed-`busy` stall — one filtered to `network,read,write` syscalls, one a full unrestricted syscall trace for a 35-second live window — show **zero syscalls of any kind** from the thread that must, per the busy-gate logic above, be alive and executing `_relay_one_stream()`'s read loop. No `recvfrom`, no `futex`, no `poll`/`select` — nothing, from any thread other than the main accept loop and unrelated short-lived HAProxy health-check threads. This is a genuine contradiction not resolved this round: the code's own logic proves the thread exists and is running; direct `strace -f -p <pid>` observation says it isn't making any syscalls at all. Possible explanations not yet tested: a `strace`/ptrace interaction quirk specific to this KVM-guest environment with an already-blocked thread at attach time; some other blocking mechanism in the CPython 3.10 socket/threading stack that doesn't surface as an ordinary syscall; or an as-yet-unidentified logic path in `verify_proxy.py` itself that diverges from a plain reading of `_relay_one_stream()`'s source.

**Root cause, found via the recommended internal `RELAY_DEBUG` instrumentation (added this same round) and confirmed directly against CPython's own `socket.py` source:** `_open_upstream_completion()` created its upstream connection with `timeout=_SOCKET_POLL_TIMEOUT_S` (5s) — not just for the initial connect, but for *every* subsequent `resp.read(1)` call in `_relay_one_stream()`'s poll loop. `RELAY_DEBUG`'s own timestamped log showed the exact mechanism directly: poll #1 legitimately waits the full 5.0s and raises `TimeoutError`, but poll #2 onward fires again after only ~0.2s (the loop's own `time.sleep(0.2)`), forever, for the rest of the connection's life. Reading `socket.SocketIO.readinto()` in CPython's stdlib explains why:
...
The *first* genuine `socket.timeout` sets `self._timeout_occurred = True` **permanently** on that file object. Every subsequent call raises immediately, without *ever* calling `recv_into()` again — meaning no further syscall is even attempted, which is exactly why two earlier independent `strace -f` sessions (documented above) saw zero syscalls from this thread: there was nothing left to trace. Since prefill alone routinely exceeds 5 seconds (F-129), that first timeout was effectively guaranteed on every real request, and from that moment on the connection was dead for reading purposes — regardless of how long real content kept arriving on the underlying, perfectly healthy TCP socket, and regardless of how large `GENERATION_STALL_TIMEOUT_S` was raised. The original code's own comment ("a real stdlib quirk... handled identically here rather than crashing the handler thread") correctly identified the *symptom* but wrongly assumed it was a retryable condition; it is not — it is permanent and terminal for that object.

Confirmed with a clean, minimal, non-networked local reproduction (two ~20-line scripts, no llama.cpp/CloudCore involved at all): a plain TCP server that sends nothing for 8 seconds (longer than a 2s poll timeout) and then sends real data. The old pattern (blocking read with a short `settimeout()`, catch-and-retry) never sees that data, exactly reproducing the live symptom. A `select()`-based pattern reads it correctly the moment it's ready.

**Fix:** two changes, both required together:
1. `_open_upstream_completion()`'s connection timeout raised from `_SOCKET_POLL_TIMEOUT_S` (5s) to `GENERATION_STALL_TIMEOUT_S + 60` — large enough that a genuine `socket.timeout()` essentially never fires in real operation, so `_timeout_occurred` is never tripped in the first place.
2. `_relay_one_stream()` now polls readiness via `select.select([sock], [], [], _SOCKET_POLL_TIMEOUT_S)` on the connection's raw socket *before* calling `resp.read(1)`, instead of relying on the read call's own timeout to interrupt it. `select()` operates on the raw file descriptor and never touches `socket.SocketIO`'s internal `_timeout_occurred` state, so it's immune to this poisoning entirely, while still providing the same periodic wakeup the original design wanted (to re-check the stall clock without blocking indefinitely). `resp.read(1)` is only ever called once `select()` has confirmed the socket is genuinely readable, at which point it returns immediately.

**Verified by:** the clean local reproduction above (both the bug and the fix, isolated from this whole deployment); then live, for real, end-to-end, exactly the way a student would use it — with the fix deployed to the running coordinator (now on llwyn-y-groes, see F-131), a real `POST /sandbox/linux-ask` request completed in 85.5s with a genuine streamed answer ("The `chmod` command is used to change the permissions of files and directories...", `finish_reason: "stop"`, real per-token timing), the first real answer this entire investigation arc (F-118 onward) ever actually produced through the student-facing path.

### F-131 — Moved the coordinator off stourport (a contended, shared desktop) onto llwyn-y-groes (a dedicated, idle host), dropping RPC in the process

**Where:** deployment/placement, not code — `coordinator_peer_id`/`coordinator_peer_vpc_id`/`coordinator_peer_subnet_id`/`coordinator_peer_security_group_id` and `worker_peers = []` at build-submission time.

**Symptom/motivation:** F-129 flagged stourport's own real CPU contention (`load average: 1.98, 2.87, 2.87` on 4 real cores, directly competing with the coordinator VM's own vCPUs) as a real contributing factor to slow throughput — a direct echo of the very reason this whole coordinator-peer-placement mechanism was built in an earlier round ("stourport simply can't run that load due to it being a dev/desktop system"). llwyn-y-groes, by contrast, is dedicated and was measured idle (`load_pct_1m: 7.4`, 8 real cores, 16.6GB available).

**Blocker found placing the coordinator there:** the peer's own existing, working VPC/subnet combo (`peering-test-vpc`) only had a security group opening ports 80/22 — not 8620 (the chat UI/API port) — so the coordinator would have been unreachable through the load balancer as-is. A separate, unrelated leftover security group did open 8620, but its own VPC's subnet had already been destroyed and wasn't reusable. Fixed by creating a fresh, correctly-scoped security group directly on the peer via this project's own existing `POST /v1/security-groups` (`peer_id` in the body) — confirmed this endpoint already supports creating security groups on an approved peer directly, no new code needed.

**Second blocker, a genuine platform-level constraint, not a bug:** `api/capacity_gate.py`'s `check_no_coordinator_worker_overlap()` deliberately, correctly rejects any build where `coordinator_peer_id` matches a `worker_peers[].peer_id` — landing both roles on the same machine defeats the entire purpose of RPC splitting. Combined with llwyn-y-groes's own real 8-core budget already being fully claimed by the worker's own `standard.2xlarge` (6 vCPU) flavor, there was no way to keep RPC *and* move the coordinator to the same host without either violating this platform rule or meaningfully oversubscribing the host. Resolved by dropping RPC entirely for this placement (`worker_peers = []`) — the coordinator now runs the whole model locally on llwyn-y-groes's own real, idle capacity, which also happens to cleanly re-test F-127's original RPC-free scenario with F-129's corrected timeout and F-130's actual relay fix both now in place.

**Verified by:** a real destroy/rebuild (old build `4794811c…` destroyed cleanly, 17 resources; new build `26027aba…` applied cleanly, 4 resources) placing the coordinator instance genuinely on llwyn-y-groes (`host_hostname: Llywyn-Y-Groes`, `192.168.101.140`), confirmed reachable via the `claude-debug` key, `llama-server.service` confirmed with no `-ngl`/`--rpc` flags at all (`-t 6`, matching the peer's real usable core count) — and this is the exact build F-130's own live fix verification ran against.

### F-132 — Retrieval-grounding added for Linux Help, fixing a real, live hallucination ("other applications run in different rings") — full-text search needed a query-extraction step to be usable at all

**Where:** `examples/llm-chat` — a new `module "kiwix"` instance (kiwix-serve, co-located with the coordinator's own peer placement) plus `verify_proxy.py`'s `_handle_linux_ask` path only (not the coding Ask panel).

**Symptom/motivation:** direct report, live: asked to "explain ring 3 in detail," the model gave a fluent but wrong answer, most seriously claiming other user applications run in "different rings" (every user app is Ring 3; only the kernel differs). Direct follow-up: "I don't feel comfortable with its current performance" — a request to reduce hallucinations at the root, not just hedge the system prompt harder (already tried; insufficient, since the model can't reliably introspect on which of its own claims are shaky).

**Design, live-tested before committing to it:** reused `examples/kiwix-library`'s existing pattern rather than building retrieval from scratch. Confirmed via direct `kiwix-serve`/`/search` testing that a real, cached ZIM already had strong relevant coverage for the motivating case ("Protection ring" ranked #1 for "ring 3 x86 privilege"), then upgraded the corpus anyway per direct request to full English Wikipedia (nopic, 52.7GB — corrected up from an initially misreported 14.4GB after checking the real catalog's `flavour`/`tags` fields more carefully) plus ManKier man pages (190MB) and ArchWiki (35.6MB), also brought into scope per direct request.

**A real problem found mid-implementation, reported rather than silently shipped:** natural conversational questions retrieved wildly irrelevant results compared to keyword-dense queries against the same corpus — confirmed live, not assumed. Stopword stripping and larger candidate pools + re-ranking were both tried and directly disproven as fixes. Flagged to the user with real before/after evidence rather than shipping a flawed feature; the user chose adding a small LLM query-extraction call ahead of the kiwix search (the student's question is first reformulated into a keyword-dense search pattern by the same model, then that pattern — not the raw question — is what's searched).

**Verified end-to-end against the real cloud deployment, the actual motivating case:** `POST /sandbox/linux-ask` with "explain ring 3 in detail" completed in 200.6s (`tokens: 447`, `2.25 tok/s` — consistent with F-129's already-documented throughput, not a new regression) and the answer no longer contains the "different rings" claim; it correctly states Ring 3 is the single, shared privilege level for all user applications, with rings 0-2 reserved for lower-level/kernel code. (One minor residual wording glitch survived — a bullet says "Ring 3 is the most privileged user mode," contradicting its own opening sentence which correctly says "least privileged" — a phrasing slip, not a repeat of the original factual error.)

**Failure-mode verified live, not assumed:** stopped `kiwix-serve.service` mid-deployment and re-asked a different question through the real path — the request still completed normally and ungrounded (143.7s, real answer content), with `verify_proxy.py` logging `kiwix search failed, answering without grounding: URLError(ConnectionRefusedError(111, 'Connection refused'))` exactly as designed. Grounding failure degrades gracefully; it never blocks or breaks a student's question.

**Two unrelated, transient infrastructure failures hit building this, not F-132 bugs:** (1) simultaneous heavy downloads (kiwix's 52.9GB corpus + the coordinator's own model/artifacts, all from the same host-level package repo) caused a real `apt-get install` failure for `libgomp1`/`promtail`/`dnsmasq`/`python3-paramiko`, compounded by the known F-045/F-052-class interactive dpkg conffile prompt for `promtail` once retried without `DEBIAN_FRONTEND=noninteractive`; (2) the small (16MB) `llama-b11025-bin-ubuntu-x64.tar.gz` archive hit `curl: (28) Connection timeout` after all 10 retries under the same repo contention, while the much larger 8.5GB model download alongside it succeeded — both fixed live via SSH (`apt-get update` once the repo was healthy again, then a plain re-run of the same pinned `curl` command, sha256-verified) rather than requiring a full rebuild that would have discarded kiwix's own already-completed download.

### F-133 — Restarting `cloudcore-api.service` on a session-mode-libvirt peer crashes every VM it's currently running, silently, mid-request

**Where:** platform/host, not application code — `qemu:///session` libvirt on llwyn-y-groes specifically (`api/compute.py` itself does the right thing: `conn.defineXML()` + `create()`, the persistent-domain path that should survive a client disconnect on ordinary system-mode libvirt).

**Symptom, reported directly:** "tell me which programming language should be used for linux kernel development" through Linux Help returned a fluent answer that stopped mid-sentence with no error shown, the UI just went back to ready — initial hypothesis was conversation history/context-window growth affecting the answer. Investigation (with the user's own direct access to the peer host, since I have no SSH credentials to physical hosts, only to guest instances via the `claude-debug` key) ruled that out and found something more concrete.

**Root cause, evidenced step by step:**
1. The coordinator VM's own `llama-server` journal showed the real answer-generation task (after the F-132 query-extraction step) launch at 23:44:27 UTC, then nothing further — the boot itself ended at 23:45:34 UTC and a fresh boot started at 23:46:01 UTC. Since `verify_proxy.py` runs on the same VM, it died in the same instant and never got to log an outcome or show an error — the browser just saw the stream connection drop.
2. Ruled out in order: the CloudCore platform itself (no stop/reboot/restart API call against this instance in the live API server's own log), the guest's own kernel (no OOM-killer or panic messages), and the physical host (`journalctl --list-boots` on llwyn-y-groes itself showed one continuous boot since 2026-09-12 — the host never rebooted).
3. Found the actual QEMU process via `ps aux` — running under `qemu:///session` (per-user, `/home/scottp/.config/libvirt/qemu/...`), not the system-wide `qemu:///system` I'd assumed from reading `compute.py`, which is why checking `qemu:///system` (with or without sudo) showed zero domains.
4. The domain's own real log (`~/.cache/libvirt/qemu/log/cc-<id>.log`) gave the exact moment and libvirt's own classification: `2026-09-23 23:45:55.060+0000: shutting down, reason=crashed`, immediately followed by a fresh QEMU process spawning — libvirt's own auto-respawn of a domain it considers crashed.
5. The host's system journal for that exact window (once the BST/UTC offset was correctly accounted for — an off-by-one-hour mistake on my first attempt at this cross-reference) showed the actual trigger: `cloudcore-api.service`, `cloudcore-terminal.service`, and `sentinel-ui.service` were all stopped and restarted within a 4-second window (00:45:48-00:45:52 BST), immediately followed by a **freshly cold-started** `libvirtd[771650]` session daemon hitting AppArmor/D-Bus "Hello" handshake failures while re-establishing itself — then the domain crash at 00:45:55 BST. Confirmed directly with the user: this was a deliberate `git pull` + restart of both `cloudcore` and `sentinel` to pick up the just-pushed F-132 fix.

**Why this happens:** unlike system-mode libvirtd (an always-on system daemon independent of any specific client), session-mode libvirtd (`qemu:///session`) is D-Bus/session-activated — effectively tied to the lifecycle of the session and the client that first activated it. Restarting `cloudcore-api.service` (the client that opened the original `libvirt.open("qemu:///session")` connection) appears to churn the session bus/libvirtd stack itself, taking down the QEMU process it was supervising along with it — even though `compute.py` uses the persistent-domain API (`defineXML`+`create`), which should be immune to this on a proper system-mode host.

**Not fixed, not a code bug to fix:** this is a real operational hazard specific to running CloudCore peers on session-mode libvirt rather than system-mode — an entirely ordinary, previously-routine action ("pull my latest push, restart the services to pick it up") silently crashes every VM currently running on that peer, with no warning to whoever is mid-request against it. Flagged rather than silently worked around; the practical mitigation for now is operational (avoid restarting `cloudcore-api.service` on this peer while it has live instances running, or accept the occasional collateral VM restart), not a code change — switching this peer to `qemu:///system` would be a real fix but is a host-level infrastructure change outside this session's scope, not undertaken without being asked. F-134 (silent-failure UX) is the direct, in-scope mitigation: make this failure mode visible to the student instead of silently truncating the answer.

### F-134 — A mid-stream connection drop (F-133's own trigger) was silently shown as a complete, successful answer — the browser had no way to tell a truncated response from a finished one

**Where:** `examples/llm-chat/files/verify_proxy.py`'s own `SANDBOX_PAGE_HTML`, `runAsk()`'s streaming-read loop (shared by both the coding Ask panel and Linux Help, since both call the same `makeAskPanel()` factory).

**Symptom:** direct report — a Linux Help answer stopped mid-sentence with no error shown, the UI just returned to ready as if the answer were finished. F-133 root-caused the actual cause (a backend VM crash mid-generation) but that's a real, if now-understood, thing that can happen again — this finding is about the client silently mishandling it either way.

**Root cause:** `runAsk()`'s read loop exits its `while` loop the moment `reader.read()` resolves with `done: true` — but a browser's `ReadableStream` resolves `done: true` for both a genuinely complete response AND a connection that was cut off mid-body; nothing about the Fetch API's own signal distinguishes them. The server does emit an unambiguous sentinel for a real completion (`data: [DONE]\n\n`, the very last thing `_do_handle_ask()` writes on every path — including its own already-handled "stalled"/"empty_response" messages), but the client never checked for it: `[DONE]` was just one more line silently skipped (`continue`), with no record kept of whether it had actually arrived.

**Fix:** a `sawDone` flag, set only when the `[DONE]` sentinel line is actually seen; if the read loop ends without it, the bubble gets an explicit, visible notice appended ("_Connection to the model backend was lost before this answer finished -- the text above may be incomplete. Please try again._") instead of silently standing in as if it were the real, complete answer. Confirmed this doesn't misfire on a deliberate Stop click: `stopAsk()` only signals the server via `/sandbox/interrupt`, it doesn't abort the client's own fetch, and the server's own "interrupted" stop_reason branch still finishes normally with its own message and a real `[DONE]` before closing.

**Verified:** Python `ast.parse` and a real `node --check` against both the raw (Python-string-escaped) and the actually-rendered JS (post Python string evaluation, the true test — confirmed the rendered output has real `\n` sequences, not literal backslash-n) both clean; confirmed the appended notice can't collide with `renderAssistantContent()`'s own code-fence splitting (plain text, no backticks). Not verified against a real, reproduced live browser mid-stream drop — deliberately reproducing F-133's own trigger (restarting `cloudcore-api.service` while a request is in flight) to test this would itself crash the live coordinator again, which wasn't judged worth doing just to exercise a client-side text-handling change; flagged honestly rather than claimed as fully proven end-to-end.

### F-135 — RESOLVED: promtail's interactive dpkg conffile prompt silently aborts the whole `packages:` install on every `llm-chat` rebuild, not just occasionally — `llama-server` then crash-loops indefinitely (5184 restarts observed) with no visible error

**Where:** all three `examples/llm-chat` cloud-init templates (`coordinator-`, `worker-`, `kiwix-cloud-init.yaml.tftpl` and their Ansible `.yml.j2` mirrors) — `write_files` always writes `/etc/promtail/config.yml` before the declarative `packages:` phase installs promtail itself.

**Symptom, reported directly:** "i've destroyed and rebuilt the llm-chat environment and we are sitting at 503 for hours now." A from-scratch rebuild, not a live incident — meaning whatever broke it was baked into the template, not a one-off contention fluke.

**Root cause, confirmed live:** `llama-server.service` was in `activating (auto-restart)`, exit code 127 (`error while loading shared libraries: libgomp.so.1: cannot open shared object file`) — `libgomp1` was never installed. `dpkg -l` showed `libgomp1`/`promtail`/`dnsmasq`/`python3-paramiko` all completely absent (not half-configured — never even unpacked), and the cloud-init log showed `package_update_upgrade_install` failing with an exception, then every later `runcmd` step depending on those packages failing too (`Failed to enable unit: Unit file promtail.service does not exist`). This is the exact same live incident already hit once during F-132's own build (previously logged there as a "transient infrastructure failure... caused by repo contention") — but it recurring identically on a completely fresh, from-scratch rebuild (not mid-build contention with another simultaneous 53GB download this time) proves it isn't contention-dependent at all: `write_files` always runs before the `packages:` phase in cloud-init's own module ordering, so `/etc/promtail/config.yml` always already exists by the time promtail's own postinst runs, and with no TTY to answer "keep the local version?" the whole apt transaction — not just promtail — aborts, silently skipping every package listed after it. `systemctl enable --now promtail` in the pre-existing `runcmd` block then fails outright (the unit was never installed), but cloud-init's own `runcmd` module doesn't halt on one failing step, so the build limps on to eventually leave `llama-server` crash-looping forever with no operator-visible signal beyond a permanent 503.

**Fix:** a new first `runcmd` step on all three templates (both Terraform and Ansible copies): `DEBIAN_FRONTEND=noninteractive apt-get install -y -o Dpkg::Options::="--force-confold" <that template's own package list>` — the same one-line fix already proven live (twice, manually, via SSH) now baked in permanently, idempotent whether the declarative `packages:` phase already succeeded or not.

**Verified:** `tofu validate`, a real YAML structural parse of all three `.tftpl` files (interpolations neutralized), and a Jinja2 parse of all three `.j2` mirrors, all clean. Live-verified the underlying mechanism directly on the broken coordinator before templating the fix: re-running the exact new command installed all four missing packages cleanly (promtail's own postinst logged `Using current old file as you requested` — confirming `--force-confold` answers the prompt correctly), `llama-server` came up healthy, and the LB's own health check (`show stat` on the HAProxy stats socket, `DOWN 1/2` → `UP` after one more 30s check cycle) confirmed the whole path healthy end-to-end.

**Not fixed, flagged separately:** `dnsmasq.service` still fails to start (`failed to create listening socket for port 53: Address already in use`, `systemd-resolved` already owns that port) — this only affects the Firecracker sandbox terminal's own DHCP/DNS bridge (`fcbr0`), not `llama-server`/Linux Help/the coding Ask panel, and was already a known, previously-flagged, not-yet-addressed gap from the original F-132 build. Left open, not silently expanded into this fix.

### F-136 — Grounding transparency: extended retrieval-grounding to the coding panel and made what was actually used, on either panel, real and browsable instead of inferred

**Where:** `examples/llm-chat/files/verify_proxy.py` (`_kiwix_search()`, new `_log_grounding()`/`_push_grounding_to_sentinel()`, both ask-handler paths) and the sibling Sentinel project (`src/sentinel/db.py`, new `src/sentinel/grounding.py`, `src/sentinel/web/server.py`, `ui/index.html`).

**Motivation, direct follow-up:** after confirming a grounded Linux Help answer (`ss`) was accurate, "are we able to tell what resources the llama server used (did we use both the llm and then corroborate with man/wikipedia?)" — the honest answer at the time was *probably, but not provably*: `_kiwix_search()` only ever logged a *failure*, never what search terms were used or what was actually found on success. Reconstructing an answer required indirect evidence (prompt token counts, live re-running the same search, eyeballing whether the model's phrasing matched real snippets) rather than a real record. Direct follow-up: "I'd like to see the search term('s) for both linux and code logged and the references made/grounding requests entered into sentinel where a user can see/check what happened with the question/answer session" — confirmed via clarifying question that this meant genuinely extending retrieval-grounding to the coding Ask panel too, not just adding logging around Linux Help's existing scope.

**Design:** `_kiwix_search()` now returns `(prompt_block, references)` — the same hits as a structured list, not re-parsed later, so what's logged is exactly what the model was actually shown. The grounding gate in `_do_handle_ask()` (previously `if endpoint_label == "/sandbox/linux-ask"`) now fires unconditionally for both panels. A new `_log_grounding()`, called once the full answer (and, for the coding panel, its own already-existing `verify_and_maybe_fix()` outcome) are known, does two things: prints a `GROUNDING_LOG` stdout line matching the existing `ASK_OUTCOME` convention (flows to Loki via the same journald→Promtail pipeline for free), and fires a best-effort, backgrounded (`threading.Thread(daemon=True)`) push to a new Sentinel endpoint. Logged unconditionally, including `grounded: false` — "nothing was found for this question" is itself real, useful data.

**Sentinel side, deliberately modeled on the *other* existing precedent, not the trouble-detection one:** `events`/`suggestions` are windowed log lines matched against known bug patterns via Sentinel's own Loki-polling watcher — the wrong shape for a plain per-request audit record. `api/scheduler.py`'s own direct pushes to `/api/kb/import`/`/api/suggestions/import` (plain `urllib`, no auth, confirmed by reading that code directly rather than assumed) were the right precedent to follow instead. New `grounding_log` table (`CREATE TABLE IF NOT EXISTS`, safe on the already-populated production DB with zero `_migrate()` changes needed), a new small `grounding.py` store module matching `kb/store.py`'s own raw-SQL style, `POST`/`GET /api/grounding-log`, and a new "Grounding" UI tab following the existing Knowledge Base tab's own expand-row pattern exactly.

**A real bug caught before it shipped, not after:** `code_verified` (the coding panel's own pass/fail outcome) was initially passed through as a bare Python bool. `grounding_log`'s own column is TEXT, and SQLite's TEXT-affinity rules silently convert an inserted `True`/`False` into the text `"1"`/`"0"` on the way in — which would have made the UI's own `=== true` comparison always false, silently showing every verified answer as unverified. Caught by working through the actual SQLite storage-class rules before testing, not after seeing a wrong UI; fixed by spelling `code_verified` out as an explicit `"passed"`/`"failed"`/`null` string end-to-end instead of relying on an implicit bool-to-text conversion.

**Verified live, end-to-end, against the real coordinator (not a synthetic test alone):** deployed the updated `verify_proxy.py` and a live-patched systemd unit (`SENTINEL_HOST`/`SENTINEL_PORT`, matching `KIWIX_HOST`/`_PORT`'s own existing pattern) without a full rebuild. Real questions through **both** panels produced accurate `grounding_log` entries — Linux Help ("what does the du command do") returned genuinely relevant hits (a real "Du (Unix)" Wikipedia article, a real `agedu` man page); the coding panel ("write a function that checks if a number is prime") returned a correct, verification-`passed` function, but its own reference hits (a GSI proxy library man page, "List of number theory topics") were noticeably weaker — real, honest confirmation of the tradeoff already flagged before this was scoped in: this corpus is a better match for systems/Linux conceptual questions than typical coding questions. Graceful degradation confirmed live too: stopped Sentinel's own service entirely, fired a real question through Linux Help, got a normal complete answer (126s, HTTP 200) with the expected `Sentinel grounding-log push failed (non-fatal): ConnectionRefusedError` logged and no added delay. Sentinel's own test suite (35 tests) re-run clean after the DB/route changes.

### F-137 — A human-vetted grounding corpus: review workflow in Sentinel (approve/reject/copy), checked first before kiwix

**Where:** Sentinel side — new `review_status`/`grounding_source` columns on `grounding_log`, `POST /api/grounding-log/<id>/status`, `GET /api/grounding-log/match`, a new "Grounding" tab UI (`src/sentinel/db.py`, `src/sentinel/grounding.py`, `src/sentinel/web/server.py`, `ui/index.html`). CloudProject side — `examples/llm-chat/files/verify_proxy.py`'s new `_local_corpus_search()`.

**Motivation, direct follow-up to F-136:** "if we start building a grounding corpus and have multiple ground[ed] and verified items, could we benefit by checking that corpus first... and then grounding in the other areas?" — confirmed, gated on an explicit human marking system ("worked as expected" / "failed inspection, remove it from the corpus") plus a copy-to-clipboard action for closer inspection elsewhere.

**Design:** `review_status` (`unreviewed`/`approved`/`rejected`, default `unreviewed`) added via the existing `_add_column_if_missing()` migration mechanism, already proven safe on the real, already-populated production DB by F-136 for the table itself. `POST /api/grounding-log/<id>/status` mirrors `update_suggestion_status` exactly. **"Rejected" excludes an entry from matching but doesn't delete the row** — an audit trail, matching how a dismissed suggestion already works; flagged as the recommended default rather than assumed. The new "Grounding" UI tab mirrors the existing Suggestions tab's own filter-row and card-action pattern exactly, down to reusing its `badge status-*` CSS classes rather than inventing new ones. Copy is client-side only (`navigator.clipboard.writeText`), no new backend route.

`GET /api/grounding-log/match` is the check-corpus-first lookup: plain word-set-overlap scoring between the query's keywords and each **approved-only** entry's own `search_terms` (both sides already keyword-dense, produced by the same `_extract_search_terms()` step every ask already runs) — cheap, deterministic, good enough while the corpus is small. FTS5 confirmed available in this project's sqlite3 build (tested live) if the corpus later grows enough to need it; deliberately not built now, since there's no real corpus yet to justify the complexity.

`verify_proxy.py`'s new `_local_corpus_search()` is called first, in `_do_handle_ask()`, right after `_extract_search_terms()` — `_kiwix_search()` only runs if nothing came back, matching the user's own stated order exactly (check the corpus first, ground in the other areas only on a miss). A local-corpus hit is labeled `"Verified Q&A"` in the injected prompt text (not a source name like `"Wikipedia"`), keeping the same "explain in your own words, fact-check against this" framing already used for every other reference — so a reused vetted answer still gets genuinely reworded for the new question's own phrasing, not copied verbatim. `grounding_source` (`local_corpus`/`kiwix`/`none`) is threaded through to `_log_grounding()`'s own pushed entry, so which corpus actually served a given answer is a real, visible field in Sentinel, not something that has to be inferred.

**Verified live, end-to-end, against the real coordinator and the real production Sentinel DB:** migration applied cleanly without disturbing existing rows; approve/reject/match all round-tripped correctly against real data (confirmed an *unreviewed* entry, even with its own exact search terms, is never returned by `match()`, and that a paraphrased query still matches via the overlap-ratio scoring). Then the full pipeline: approved a real past entry ("what does the du command do"), asked a paraphrased version of the same question through the real coordinator, and confirmed the local corpus fired (`grounding_source: "local_corpus"`, reference labeled `"Verified Q&A"`) with kiwix correctly skipped entirely — and the model's new answer was genuinely reworded, not a verbatim copy of the stored one.

### F-138 — Added a curated Python/coding grounding corpus (~4.1GB) to kiwix-serve, and caught a real curl resume bug doing it

**Where:** `examples/llm-chat/files/kiwix-cloud-init.yaml.tftpl` + `variables.tf`/`locals.tf` (and Ansible mirrors), `api/build-package-repo.sh` — the same `kiwix-serve` instance F-132/F-136/F-137 already built.

**Motivation, direct follow-up to F-136/F-137:** F-136's own live testing already showed the coding panel's references were noticeably weaker than Linux Help's (a GSI proxy library man page for a "is this number prime" question) — Wikipedia/ManKier/ArchWiki simply have no real Python-specific content. Direct follow-up: "are there any other downloadable resources for grounding... especially in the python field for the coding area?"

**Curation, against the real Kiwix catalog, not a guess:** queried `library.kiwix.org/catalog/v2/entries` directly (same anti-bot-gated endpoint F-132 already established how to reach) for Python and DevDocs-tagged entries. Per direct instruction ("grab as much as we can, but only quality products... anything with flimsy reputation isn't required"): included `docs.python.org_en_all` (official Python docs, ~3.96GB) and `peps.python_en_all` (official PEPs, ~9MB), plus a DevDocs bundle (~110MB: python, numpy, pandas, django, flask, fastapi, matplotlib, scikit-learn, requests, jinja, click, pygame — DevDocs mirrors each project's own official docs verbatim, not third-party writeups). Explicitly excluded: the Corey Schafer tutorial-video ZIMs also in the catalog (up to 2.5GB for as few as 7 "articles" — thin text content relative to size, a poor fit for full-text search grounding regardless of the presenter's own legitimate reputation — a fitness-for-purpose call, not a reputation one), two small personal-site crawls with unclear editorial authority, and the remaining ~219 non-Python DevDocs entries (the sandbox only ever executes Python).

**A real bug caught mid-download, not after:** the one-off bulk-download script written for this round was missing curl's `-C -` (resume) flag — unlike the project's own established cloud-init download-loop convention, which already includes it. On the 3.96GB `docs.python.org` file, a stall at 1.5GB in triggered a retry that silently restarted from byte 0 instead of resuming, and a second stall did the same — meaning the configured 5-attempt retry loop could plausibly have exhausted itself and never completed a file this large over a connection with any stalls at all. Caught by comparing file sizes across two check-ins (1003MB, then inexplicably 269MB) rather than assuming the retry loop was working as designed; fixed by adding `-C -` and confirmed the very next resume attempt correctly picked up from the existing partial file rather than restarting.

**Verified live, not just "the file exists on disk":** all 14 files downloaded and sha256-verified locally (each size cross-checked against the real catalog's own reported sizes, all matching closely); deployed to the running kiwix instance without a rebuild (`scp` + a second sha256 pass on the remote side, confirmed byte-for-byte identical, before moving into place); `kiwix-serve.service`'s `ExecStart` updated to load all 17 books together and restarted cleanly. Confirmed the pre-existing corpus still works (`ring 3 x86 privilege` still returns 200) and, more importantly, that each new corpus is genuinely indexed and returning real, on-target hits — not just present on disk: `numpy.reshape()`/`numpy.ravel()` for "numpy array reshape", Django's queryset/model-field docs for "django model queryset", PEP 318 for "python decorator function", and the official docs' own `ast` module page for "list comprehension syntax".

### F-139 — A long pasted error timed out search-term extraction, silently producing zero grounding right when it was most needed; also live-confirmed F-137's own approved-but-wrong propagation risk for real

**Where:** `examples/llm-chat/files/verify_proxy.py` — `_extract_search_terms()`, `_SEARCH_TERMS_TIMEOUT_S`, a new `_SEARCH_TERMS_MAX_QUESTION_CHARS`, a new appended student-facing note, and a new "Clear session" button on the Linux Help panel.

**Motivation, direct report:** a student pasted a full multi-line `pip install` error (`externally-managed-environment`) into Linux Help and got back an answer with a real, confidently-stated factual mistake — "meaning it's not under the control of the system's package manager (apt)" is backwards (confirmed live against a real Ubuntu box's own `EXTERNALLY-MANAGED` marker file: the environment *is* managed by apt, external to pip's own view, which is exactly why pip refuses to touch it). A second, smaller error was also caught: `sudo apt-get install -y python3-pipx` — the real package is just `pipx` (confirmed live via `apt-cache policy`; `python3-pipx` doesn't exist).

**Root cause, found by reading the actual server log, not assumed:** `verify-proxy: search-term extraction failed, using raw question: TimeoutError('timed out')`. Prefill time scales with input length, and the pasted error was a few hundred characters longer than a typical question — enough to push the old 30s timeout (tuned for a short question) past its limit on this hardware's own measured prefill throughput (as low as ~4.75 tok/s, see F-129). On timeout, `_extract_search_terms()` falls back to the raw, still-long question as the literal search pattern — a poor full-text query on its own (the original "ring 3" investigation's own finding about excess words diluting ranking) — so the request got zero grounding, silently, at exactly the moment a long/unusual question most needed a fact-check.

**Fix:** `_SEARCH_TERMS_TIMEOUT_S` raised 30 → 90 for real headroom; a new `_SEARCH_TERMS_MAX_QUESTION_CHARS` (1000) truncates the text handed to *this* extraction call specifically (bounding worst-case prefill rather than just chasing an unbounded paste with an ever-larger timeout) — the model itself still sees and answers the student's full, untruncated question regardless; only what's used to search for reference material is affected. When truncation happens, a visible note is now appended to the answer explaining this plainly and recommending the student paste just the most relevant part of a long error next time, rather than leaving a silent quality drop the student has no way to notice.

**Verified live against the real coordinator with the exact original failing input:** extraction now succeeds (real extracted keywords: `"pip install flask flask-socketio externally-managed-environment virtual environment python3 -m venv pipx"`, no timeout), response time dropped from timing out past 400s to a genuine 81s, and the truncation warning rendered correctly, appended after the answer.

**A real, live-demonstrated confirmation of F-137's own known propagation risk, not just a theoretical concern:** re-running the same question after the fix reused the *original wrong answer* as grounding — because that entry had been approved in Sentinel (deliberately, by direct instruction, specifically to test whether the review-gate mechanism would catch and propagate a bad approval exactly as designed: "sometime I want to put things in the way to make sure we're keeping on track"). `find_match()` behaved correctly (only approved entries are ever returned) — the human approval itself was wrong, and the system faithfully trusted it, exactly the risk flagged when F-137 was first scoped. Rejected via Sentinel's own `POST /api/grounding-log/<id>/status` once confirmed; a repeat question should now fall through to kiwix's own `docs.python.org`/PEPs coverage (added in F-138) instead.

**Also added, same round, direct request:** a "Clear session" button on the Linux Help panel, mirroring the coding panel's existing one — deliberately still a *separate* action (the two conversations stay independent by design, same reasoning the coding panel's own button already documents), not a single button that wipes both.

### F-140 — A third grounding tier: CloudCore's own source code, indexed without a ZIM-style rebuild-and-redeploy cycle, and two real FTS5 query bugs caught before calling it done

**Where:** Sentinel side — `sentinel/src/sentinel/db.py` (new `codebase_fts` FTS5 virtual table), new `sentinel/src/sentinel/codebase_index.py`, `sentinel index-codebase <path>` in `cli.py`, `GET /api/codebase-search` in `web/server.py`. CloudProject side — `examples/llm-chat/files/verify_proxy.py`'s new `_codebase_search()`.

**Motivation, direct follow-up to F-136/F-137/F-138/F-139:** "can we include this repo's codebase... in the ask the model and linux help. the reason i ask is that it's now extensive and for the most part... it works and works well so could provide some very detail[ed]" answers. Scoped down on a follow-up to the actual source code (Python/Terraform/Ansible/shell/Go), not ZIM packaging and not the findings log. One hard requirement, stated explicitly: **"ensure it's easily updated... i'd would hate for it to be a monster chore to update"** — this codebase changes on nearly every commit in this project, unlike the static reference corpora already loaded.

**Design, deliberately not the existing kiwix/ZIM mechanism:** every existing corpus is a static archive rebuilt rarely; a ZIM covering this repo would need a full rebuild-and-redeploy cycle on every meaningful change — exactly the "monster chore" being avoided. Instead: a SQLite FTS5 index hosted in Sentinel (same home as F-137's own local Q&A corpus), refreshed by one command — `sentinel index-codebase <path>`, mirroring `sentinel ingest-kb`'s own already-familiar CLI shape exactly. Deliberately a full `DELETE`-then-reinsert on every run, not incremental sync: confirmed live the real target repo is only 334 matching files / 3.3MB (a whitelist of source extensions, with `.git`/`.venv`/`__pycache__`/this project's own known-huge `api/package-repo`/`api/images`/`api/instances` directories pruned from the walk entirely, not just filtered after listing) — a full rescan measured at ~2.5s, cheap enough that there's no correctness-vs-performance tradeoff worth the complexity of tracking deletes/renames. Wired into `verify_proxy.py`'s existing check-order as a new middle tier: human-approved Q&A (F-137) → **codebase** → kiwix (broadest, last resort) — a codebase hit is narrower/more specific than general reference material, so it should win over Wikipedia/DevDocs when both might match, but a human-approved answer still wins over everything. `grounding_source` gains the value `"codebase"`; references are labeled `"CloudCore source"` with the real file path as the title. No Terraform/Ansible/cloud-init changes needed at all — `SENTINEL_HOST`/`SENTINEL_PORT` were already wired into the coordinator's own environment since F-136, so this is purely a new endpoint on the same already-reachable Sentinel instance.

**Two real bugs caught live before calling this done, not after shipping:**
1. A query containing a hyphenated technical term ("force-confold") came back with zero results — FTS5's own query syntax reads a leading `-` as a NOT operator, so it silently excluded "confold" rather than matching the literal compound word. Fixed by tokenizing the query into plain words and double-quoting each one, turning every token into a literal phrase match immune to FTS5's own special syntax.
2. Even after that fix, a realistic 5-word extracted search-terms string ("promtail cloud-init installation failure") still came back empty — FTS5's own default behavior ANDs bare terms together, and real source comments don't repeat a student's exact phrasing verbatim (they say "aborting"/"conffile prompt", not literally "installation failure"), so requiring every extracted word to appear in the same file was far too strict. Fixed by OR'ing terms together and relying on `bm25()` ranking instead — the same best-effort, ranked-not-exact shape kiwix's own Xapian search and `grounding.find_match()`'s own overlap scoring already use.

**Verified live, end-to-end, through the real student-facing path, not just direct endpoint tests:** after both fixes, asked "explain the force-confold dpkg conffile fix for promtail in this project" through the real Linux Help endpoint on the live coordinator — confirmed `grounding_source: "codebase"` fired (not kiwix), with real file paths (`worker-cloud-init.yaml.tftpl` and its Ansible mirror) correctly cited as `"CloudCore source"` references, and the resulting answer was specifically and accurately grounded in this project's own real fix, not generic packaging knowledge. An earlier run with a more generic question ("why did promtail fail... and how was it fixed") fell through to kiwix instead once the codebase tier came up empty on overly generic search terms — an accepted, expected limitation (the same "full-text search is sensitive to phrasing" lesson already learned for kiwix in F-132), not a bug.

### F-141 — Codebase index chunked by logical unit, not whole file: fixed the two retrieval gaps F-140's own live tests exposed, and confirmed search-term quality is now the real bottleneck

**Where:** Sentinel side — `sentinel/src/sentinel/db.py` (`codebase_fts` gains a `start_line` column), `sentinel/src/sentinel/codebase_index.py` (new `_chunk()`), `cli.py`'s `index-codebase` print. CloudProject side — `examples/llm-chat/files/verify_proxy.py`'s `_codebase_search()` reference title.

**Motivation, direct follow-up to F-140 via two real live UI tests:** asked "why does the coordinator use a different compute flavor than the RPC workers" and got a generic, hedged answer grounded on `api/capacity_gate.py`/`14-llm-chat.yml`'s own header comment rather than `examples/llm-chat/variables.tf`'s genuinely detailed sizing comment. Separately asked "how does CloudCore decide how many layers to offload... and why isn't that hardcoded" and got an answer that correctly named `compute_offload_layers` and its parameters but missed the two richest "why not hardcoded" reasons (real GGUF-derived layer count, live-capacity-weighted splitting) — the function's own docstring, not a call site. Both traced to the same root cause: one row per whole file in `codebase_fts`, so (a) a large multi-topic file's own best content got diluted against everything else in it under `bm25()`, and (b) FTS5's `snippet()` windowed around wherever the matched term happened to sit, not the richest explanatory part.

**Design:** a single blank-line-based `_chunk()`, deliberately not five per-language parsers (Python `ast`, an HCL brace-matcher, etc.) — this repo's own comments already sit directly above the code they explain, with a blank line separating unrelated blocks, so one universal chunker keeps a docstring/comment together with its function across every included language. `CHUNK_MAX_CHARS = 800`, conservative on purpose: two chunks (the existing top-2 cutoff) is ~400 tokens of added prompt, and this hardware's own measured slow prefill throughput (F-129) makes prompt size a real latency cost. `codebase_fts` gains a `start_line UNINDEXED` column for honest per-chunk provenance (`path:line` as the reference title, not a claim about the whole file); since the whole table is rebuilt from scratch on every `index-codebase` run anyway, the migration simply drops and recreates it rather than altering in place. `search()` now returns each chunk's full content directly rather than a further-truncated `snippet()` window, since a chunk this size already *is* the reference material.

**Verified live — a genuine, honest mixed result, not a clean win:** direct `/api/codebase-search` tests with the right query vocabulary confirm the fix works exactly as designed — `compute_offload_layers`'s own docstring (GGUF-derived layer count, live-capacity-weighted splitting) now comes back at #1 for a query using its own real terms, and `examples/llm-chat/variables.tf`'s real Qwen2.5-Coder-14B/8.37GB sizing story plus `api/layer_split.py`'s own "47 of 48 layers, real OOM" incident comment both surface cleanly for a query using their vocabulary. But re-asking the exact coordinator/worker-flavor question through the real Linux Help endpoint post-deployment did *not* surface that ideal content — the model-generated search-term extraction that round landed on RPC-wiring vocabulary ("compute flavor CPU kernel") rather than sizing/capacity vocabulary, so the answer correctly and honestly said the reference material didn't address flavor selection, instead of confidently speculating (an improvement in honesty over the pre-chunking answer, which speculated without flagging the gap). Confirms chunking traded whole-file recall generosity for chunk-level precision — search-term extraction quality is now the binding constraint on this tier's answer quality, a separate, smaller, not-yet-built lever (nudging extraction toward function/variable-shaped terms) flagged for later, not addressed this round.
...
| v0.74 | 2026-09-22 | Paul Scott | Direct follow-up prompted by F-113: asked whether "Run in Terminal" should pre-check if a suggested command's tool needs installing before running it. Recommended against a real pre-check mechanism (detecting which binary an arbitrary, possibly multi-command shell block depends on is fundamentally unreliable) in favor of teaching the model to build its own install-if-missing check into the one command it suggests — approved, and iterated live rather than shipped on a first guess. First wording ("build a check into the SAME command") got real, partial compliance: asked about `ifconfig`, the model correctly recognized it might be missing and offered an install step, but as a *second, separate* fenced block rather than combined with the real command — still leaves the student hitting "command not found" on the first (obvious) block. Tightened to an explicit "give ONLY ONE block, do NOT also show a plain/naive version" instruction and re-tested against two more real, different questions (`traceroute`, `htop`): both came back as exactly one correctly-combined `command -v X >/dev/null || sudo apt-get install -y X; X` block, confirming the fix generalizes rather than only fitting the one worked example in the prompt itself. `linux_system_message` (Terraform/Ansible default) and the live coordinator's `linux-system-message.txt` updated and redeployed twice as the wording was refined, each round verified against a real `/sandbox/linux-ask` round trip rather than assumed correct from the text alone. |
| v0.75 | 2026-09-22 | Paul Scott | Stage 9 — direct request: "is there a way we can tie the run in terminal back to the llm and check for it's success. if no success offer a fix and allow another run in terminal and so on until fixed" — explicitly *not* autonomous re-execution, confirmed directly ("run, fault, offer alternative, offer run in terminal again was my idea"). Every suggested command sent via "Run in Terminal" now carries a randomized completion sentinel (`echo "MARKER:$?"`, appended after the existing heredoc); the browser polls the real, already-rendered `xterm.js` buffer (not the raw WS byte stream, which still carries ANSI/bracketed-paste codes) for up to 45s, and on a genuine non-zero exit, automatically relays the real command, real transcript, and real exit code back to the model as a plainly-labeled `[Automatic -- ...]` turn asking for a fix -- success is reported quietly with no model call, and a timeout (a genuinely long-running/interactive command) says so honestly rather than claiming success or failure. A new `askWithText()` on the shared `makeAskPanel` factory (askModel() itself now a thin wrapper over it) is what lets this system-constructed turn enter the Linux Help conversation exactly like a real question would, with no new backend route needed -- the "diagnosis" is just another turn through the existing `/sandbox/linux-ask`. F-115: two real, self-introduced bugs found and fixed via live testing before shipping -- the sentinel-matching logic originally misdiagnosed every command, success included, as a failure (matching the shell's own echo of the typed command before a follow-up regex fix required real digits after the colon); the first attempt at that regex fix then broke the live page's script entirely with a real SyntaxError, caught immediately and root-caused by importing the actual Python module and reading its real runtime string (after an earlier bash-heredoc-based verification attempt gave misleading results from yet another uncontrolled escaping layer). Full detail, including the raw-WebSocket verification methodology and the headless-Chrome CDP rendering-latency artifact encountered along the way (concluded to be a test-automation-specific quirk, not a product defect), in F-115 itself and the phased-implementation doc's own Stage 9 section. |
| v0.76 | 2026-09-22 | Paul Scott | Automatic best-host placement for `examples/llm-chat`'s coordinator, prompted by a direct observation (`llama-server` using 87% of the coordinator's 4GB RAM on `standard.large`) that led to clarifying the real lab topology (stourport, the shared desktop, vs. llwyn-y-groes, a dedicated 8-vCPU/32GB testing host reachable as an approved peer) and a direct request: "check which of the machines has the most resources to offer... if one can offer 16gb 6 vcpus 100gb disk use that, or use as close to it as it can be. and it should be any machine not locked to one specific." Live migration of an already-running VM was explicitly named and deferred indefinitely ("we'll leave live migration for later, if ever") — out of scope here. New `standard.2xlarge` flavor (`api/compute.py`, 6 vCPU/16GB/100GB) — 6, not llwyn-y-groes's full 8 cores, per direct correction: "i don't think we should assign all cores to one size. we should leave 2 cores free for o/s overhead etc." (`[[feedback-vm-core-reservation]]`). `api/capacity_gate.py` gained a real, three-axis (RAM/vCPU/disk) `affords()` check and a `best_fit()` search trying flavors largest-first across every candidate host, both reusing the existing RAM headroom factor and a new flat `HOST_RESERVED_CORES = 2` applied to any host, not just llwyn-y-groes specifically. `api/peers_routes.py`'s existing `GET /v1/peers/recommend-placement` gained an optional `flavor_candidates` param (fully backward compatible — omitting it reproduces today's exact response, confirmed live), annotating every candidate host with the largest flavor it specifically affords. `ui/src/js/20-tofu-manager.js` wired this up end to end: `coordinator_peer_id` now genuinely auto-selects (previously always forced local whenever `worker_peers` coexisted on the form, to avoid recommending a peer a worker checkbox was about to auto-claim) via a new client-side exclusion-aware re-rank among the server's per-host `best_flavor` annotations, and a new `coordinator_flavor` field pre-fills to match — both gracefully falling back to today's exact local/schema-default behavior when no candidate affords even `standard.large`. F-116: live-testing `affords()` against llwyn-y-groes's real stats caught the first vCPU-check implementation multiplying live CPU load on top of an already-maximal static reservation, which made the deliberately-tight `standard.2xlarge` (sized to exactly saturate `cores - HOST_RESERVED_CORES` when idle) fail under any nonzero background load, however small — fixed by making the vCPU check purely static, keeping live load only as `best_fit()`'s existing tie-break signal. Verified live end to end: the real API server restarted and re-queried (`recommend-placement` both with and without `flavor_candidates`, confirming full backward compatibility and correct per-host `best_flavor` values against real live stats); `ui/build.sh` re-run to bundle the frontend changes into the served `index.html` (confirmed via `send_from_directory`'s own no-cache serving — no separate restart needed); the exact new `coordinatorPick` selection algorithm exercised against real fixture data pulled live from the backend plus three synthetic edge cases (worker-claims-the-only-affording-peer graceful fallback, larger-tier-beats-lower-verdict-severity, and same-tier verdict tie-break), all passing; `16-build-manager.js`/`28-scheduler.js` regression-checked to confirm neither references any of the new symbols nor passes `flavor_candidates`, so both are unaffected. A full in-browser click-through (headless Chrome via CDP) was attempted but abandoned — Chrome could be launched fine in a plain foreground call, but was reliably killed (exit 144) the moment it was backgrounded for a longer-running interactive session, in every variant tried (bare `&`, `run_in_background`, `--no-zygote`, sandbox disabled) — concluded to be a sandbox restriction on this session's own tool environment rather than anything about the code under test. |
| v0.77 | 2026-09-22 | Paul Scott | Direct report from a real fresh `llm-chat` build attempt (a newly rebuilt lab environment, so the first full `tofu apply` to exercise `preview_ports` since it was added): `tofu apply` failed with `Error: Missing expression ... for var.preview_ports ... found the end of the file`. F-117: `api/tofu_engine.py`'s `extract_template_vars()` used a plain `\S+` regex fallback to capture a variable's `default = ...` value, which stops at the first internal whitespace — `preview_ports`'s own real default, idiomatically formatted (`default = [41001, 41002, 41003, 41004]`), was silently truncated to `"[41001,"`, an invalid unterminated list that flowed straight through the schema into the submitted build's `TF_VAR_preview_ports` and broke `tofu apply` outright. Reproduced exactly via a standalone `tofu plan` with the truncated literal before fixing. Fixed with a new `_extract_default_value()` that handles a quoted string, a bracketed list/braced object via real depth-tracking (reusing the same technique `_extract_balanced_block()` already uses for the surrounding variable block), and the original bare-token fallback, in that order. `api/build_engine.py`'s own separate Ansible-template variable extractor checked and confirmed unaffected (real YAML parsing, not regex). A repo-wide scan of every example's `variables.tf` confirmed `preview_ports` was the only variable actually corrupted by the old regex. Verified live: isolated re-extraction confirms the correct full default; a standalone `tofu plan` with the corrected value parses cleanly; the real API server restarted with the fix, and `GET /v1/tofu/templates/llm-chat/vars` confirmed returning the corrected default over HTTP, with F-116's own `recommend-placement` fix re-confirmed unaffected by the same restart. |
| v0.78 | 2026-09-22 | Paul Scott | Direct report: "if you use linux help and ask a question we often find the llm running out of steam or trailing off. if you push for it to finish it will think a while and then probably finish it." Root cause traced to `llama-server`'s own `-c 4096` context-window ceiling (not the model choosing to stop) — its OpenAI-compatible endpoint's final streamed chunk carries `finish_reason: "length"` whenever this happens, which `verify_proxy.py` never used to even read. `_relay_and_verify_stream()` now automatically fires up to `MAX_CONTINUATION_ROUNDS` (3) "continue" rounds on that signal, relayed seamlessly into the same SSE stream — no frontend changes needed, confirmed by reading `20-tofu-manager.js`'s own SSE consumer, which already just accumulates `delta.content` until the server closes the connection. F-118: the first, purely-instructional continuation prompt made the model restart its whole answer from scratch instead of resuming, risking real duplicated content — fixed by quoting the literal tail of the prior text back at the model instead of trusting it to remember where a separate completion left off; re-verified clean under a deliberately brutal 15-token-per-round forced truncation. Chasing this further with a real, un-forced question surfaced something much more serious — F-119: `llama-server` can silently deadlock when RPC offloading (`worker_peers`) is active, wedging the ENTIRE coordinator for every future request, confirmed via direct kernel-stack inspection (`/proc/<pid>/task/*/stack`) on both the coordinator and the RPC worker: the worker sits healthily idle waiting for a request that never comes, while every `llama-server` thread except its HTTP listener sits genuinely stuck in `futex_wait` — a real bug in `llama-server`/`ggml-rpc` itself, not this deployment's own code, and confirmed to recur on the large majority of real requests sent during this session's own testing, not a rare edge case. Built a self-healing watchdog (`GENERATION_STALL_TIMEOUT_S`, default 120s, threaded through Terraform/Ansible) that tracks time since the last *real content byte* (not just any byte — `llama-server`'s own keep-alive pings mask a naive connection-timeout from ever firing) and automatically restarts `llama-server.service` in the background once exceeded, telling the student honestly rather than leaving them hanging forever; found and fixed a real crash bug in the watchdog's own first implementation along the way (a `socket.timeout`-only except clause missed the plain `OSError` `http.client`'s buffered reader raises on the read immediately after the first timeout, caught via a real journal traceback during testing). Verified live and autonomously: the watchdog fired correctly four separate times during real-usage testing, each waiting the exact configured 120s and successfully restoring the coordinator to health; also confirmed (and documented, not worked around) that `verify-proxy.service`'s own pre-existing `Requires=llama-server.service` means this restart briefly bounces verify-proxy itself too — a low-cost, few-second side effect. Given how frequently the deadlock recurred this session, flagged directly to the user as a live concern worth further attention (dropping RPC offloading, or awaiting an upstream llama.cpp fix), not just a theoretical safety net. |
| v0.79 | 2026-09-22 | Paul Scott | Follow-up to F-119: discussed dropping RPC offloading (rejected — losing the cross-host compute split isn't worth it given llwyn-y-groes's own spare capacity could instead just run the whole model there) and swapping llama.cpp for another serving engine entirely (also rejected — vLLM/TGI/TensorRT-LLM's own multi-node support targets GPU clusters with fast interconnects, not two ordinary CPU boxes on a LAN, and Ollama/llama-cpp-python/koboldcpp all wrap the same underlying llama.cpp/ggml engine so would likely inherit the same bug class; "it doesn't make sense with this limited h/w to start trying to do better with unknowns and low confidence"). Checked whether a newer llama.cpp release (currently pinned `b11025`) fixes the deadlock: no relevant commits landed in `ggml-rpc.cpp`/`transport.cpp` through today's absolute latest (`b11115`), but the exact bug is already reported upstream, unaddressed — [ggml-org/llama.cpp#28908](https://github.com/ggml-org/llama.cpp/issues/28908): the RPC worker's `accept()` loop is single-threaded, so a second concurrent connection from the same client while a first stays open starves forever waiting for a HELLO reply that can never come, matching this deployment's own `/proc/<pid>/task/*/stack` evidence closely. A candidate fix exists ([#28916](https://github.com/ggml-org/llama.cpp/pull/28916)) but is unmerged, under maintainer pushback on the approach — not adopted, per the same low-confidence-avoidance reasoning. Given that root cause (a SECOND concurrent connection to the same worker triggers the starvation), a direct request followed: check whether the assistant is busy before ever sending a question upstream at all, reject a second/third/whatever concurrent question outright (thrown away, not queued) rather than let it collide, and actively tell that student once the coordinator is free again rather than leaving them to guess-and-retry. New global `_llm_busy` gate in `verify_proxy.py`, checked after the existing per-IP `ask_active` check (so a student re-clicking their own in-flight question still gets the friendlier existing message) but before ever opening a connection to `llama-server` — a second student's request is rejected with a new, distinct 503 (not reusing 429, so the browser can tell this case apart from the existing rate-limit/already-active ones) and never reaches the upstream connection that could trigger #28908 at all. New `GET /sandbox/ask-status` (trivial, instant, no LLM call) lets the rejected student's own browser poll for availability only after being told busy, showing "Available again" via the existing status line the moment the gate clears — no queuing, no auto-resubmission, matching the direct request precisely. Verified: both the real and `ansible/` mirror copies compile cleanly and the embedded JS passes `node --check`; live end-to-end verification against a running coordinator was not possible this round — the lab environment was torn down (all builds show `destroyed`, no live instances) partway through, mid-session, independent of this change — flagged plainly as not yet live-tested, pending a fresh build. |
| v0.80 | 2026-09-23 | Paul Scott | Live verification of v0.79's own busy gate against a freshly rebuilt coordinator: a genuinely distinct second request (spoofed `X-Forwarded-For`, since a real second physical client wasn't available to test from) was correctly rejected with a clean 503 before ever reaching `llama-server`, and `GET /sandbox/ask-status` correctly tracked `busy: true → false` across the whole cycle, including through the brief `verify-proxy.service` restart window the `Requires=llama-server.service` dependency causes. One minor, non-blocking nuance noted: `ask-status` can report "free" a few seconds before `llama-server` has actually finished reloading post-restart — a retry in that narrow window gets the existing clean "unreachable" message, not a hang, so left as-is. Separately: the user reported a question left running overnight (~8+ hours, browser tab stuck on "Thinking…") — investigation of the real overnight journal found the coordinator itself was healthy and idle, not stuck; a genuine `llama-server` crash (`ggml_abort()`, not the usual F-119 silent deadlock) had happened and self-healed via systemd's own `Restart=on-failure` within about a minute, but the student's own browser was simply left with a dead connection and no error, since the crash's own cascading restart (via `Requires=llama-server.service`) likely killed `verify-proxy.service` mid-request before it could report anything — a real, distinct gap from what the F-119 watchdog covers, logged as a known limitation rather than fixed this round. F-120: chasing the crash's own root cause found the RPC worker had only ~1.7GB free out of 7.8GB total at the time, with the rpc-server process itself already using ~5GB for its own share of the 14B model — `worker_flavor` had been sized to `standard.xlarge` (8GB) by matching the peer's real core count back when `worker_flavor` was first split from `coordinator_flavor`, before the default model was later stepped up from 7B to 14B in a separate change that never revisited the worker's own RAM sizing. Raised to `standard.2xlarge` (16GB) in both the Terraform and Ansible copies, confirmed live via `GET /v1/peers/<id>/stats` that the underlying host has 17GB+ genuinely available. F-121: that change would have silently failed at `tofu apply` — the OpenTofu provider's own flavor validators (`instance.go`, `nfs_server.go`) had never learned about `standard.2xlarge` at all, the same class of gap F-099 already fixed once for `standard.xlarge`; fixed in both files, provider rebuilt and reinstalled into the dev-overrides plugin path, `tofu validate`/`go build`/`go vet` all confirmed clean. Not yet verified end-to-end against a real worker actually running at the new, bigger flavor under real load — would need a fresh build. |
| v0.81 | 2026-09-23 | Paul Scott | Live verification of F-120's flavor fix against a freshly rebuilt worker: `free -h` confirmed 8.4GB available (vs. the old ~1.7GB) running at `standard.2xlarge`, and a real end-to-end question completed successfully — confirming the memory fix genuinely works. A separate F-119 stall (the unaffected upstream threading bug, recurring exactly as expected since RAM size was never its cause) was also observed and self-healed correctly by the watchdog during the same session, confirming the two issues are properly independent. A real student report followed: a fresh question in a brand-new private tab (ruling out stale browser history) rendered literal SSE protocol text (`data: [DONE]`) as the visible answer. Live debugging with the user via real browser DevTools (Network + Console, watched live rather than guessed at) confirmed the request itself fires correctly and reaches the server -- a genuine stall reproduced live and resolved correctly with the expected honest message, proving the watchdog and frontend work together properly end to end. F-122: chasing why F-119 stalls recur so often even with the busy gate in place (which only guards against two *different* students colliding) found a real, separate, self-inflicted bug -- `_do_handle_ask()`'s own initial connection to `llama-server` stayed open through every F-118 continuation round instead of closing once its own response was fully read, meaning a multi-round answer could have two connections to `llama-server` open at once -- exactly the condition #28908's own starvation bug is triggered by, self-caused by the continuation feature rather than just bad luck. Fixed by closing the connection immediately once its own read completes, matching how every continuation round already closes its own connection promptly. Deployed live; not yet confirmed to measurably reduce real-world stall frequency, and the specific `data: [DONE]` text artifact itself remains unexplained -- both flagged honestly as open rather than claimed fixed. |
| v0.82 | 2026-09-23 | Paul Scott | Chasing v0.81's own still-open `data: [DONE]` mystery: the student traces it to going through the local LB URL (`http://127.0.0.1:8620/`, CloudCore's own deliberate stable, rebuild-surviving `chat_url` output routed through HAProxy) versus hitting the coordinator's real IP directly, where it "immediately responded" — a real, fully-answered question, confirmed correct and notably fast. Direct SSH debugging of the LB config (`api/lb/<id>.cfg`) found `timeout client/server 300s` (already a hard-won fix from a prior incident) far exceeds the observed ~82s failure window, so that wasn't it; a live A/B test (one request via each path, fired at the exact same moment) instead caught the real cause directly: HAProxy correctly returned a clean `503 Service Unavailable` when no backend was available, while the *direct* path got accepted then silently returned nothing — flipping the original hypothesis that the LB was the less honest path. F-123: root-caused to a real, narrow race in `verify_proxy.py` itself — an upstream connection to `llama-server` that connects successfully but has its body close with zero bytes (landing in the split-second a restart-cascade is tearing the process down) was silently treated as an ordinary content-free "success"; fixed with an explicit `empty_response` outcome and its own honest, correctly-differentiated message ("try again", not "restarting", since this is a momentary window, not a hung process). Verified live by deliberately reproducing it twice — polling `/health` to catch the exact instant a real restart began, then firing a concurrent burst right at that moment; two requests landed in the race window and correctly got the new message instead of the old bare `data: [DONE]`, the other three correctly hit the busy-gate's own rejection, confirming both mechanisms work together. Separately, per direct request for better observability given how much of this investigation required live-in-the-moment SSH digging: F-124, HAProxy previously had no `log` directive at all (confirmed live: `/var/log/haproxy.log` existed, completely empty) — fixed by emitting `log /dev/log local0`/`option httplog` in every generated LB config, using Ubuntu's own pre-existing `/etc/rsyslog.d/49-haproxy.conf` rule rather than standing up new syslog plumbing; and a new `_log_ask_outcome()` in `verify_proxy.py` emits one structured JSON line per ask request covering every real outcome (`success`/`stalled`/`disconnected`/`interrupted`/`upstream_unreachable`/`length_exhausted`/`empty_response`), not just the success case `_record_llm_stats()` already covered. Both verified live: a real graceful HAProxy reload picked up the new config and produced real access-log lines; the structured outcome log correctly recorded a real stall's exact duration (120.12s, matching `GENERATION_STALL_TIMEOUT_S` to the hundredth) across two separate process restarts. Deeper answer-quality/hallucination monitoring was explicitly discussed and deferred as a separate, larger initiative, not folded into this round. |
| v0.83 | 2026-09-23 | Paul Scott | Direct request to make actual use of F-124's own new logging: confirmed live, via a direct Loki query, that `verify_proxy.py`'s new `ASK_OUTCOME` lines were *already* flowing coordinator → journald → Promtail → Loki fully labeled with zero new work, since the Lab VM's own `journal` scrape job already captures every systemd-managed unit — and that Sentinel (a sibling project, already a standing 5-day always-on watch-loop + web UI at `localhost:8900`, not something needing to be built) was already running and already querying that same Loki. Two real gaps closed: (1) HAProxy itself runs on stourport, not a Lab VM, so its own new access log had no shipping pipeline at all — `api/setup-host-promtail.sh` added, installing a dedicated host-level promtail instance from the same cached `.deb` this project already uses for Loki/Grafana, scoped to `/var/log/haproxy.log` (prepared, needs the user's own sudo to actually run — not yet applied). (2) Sentinel's own rule-based signal layer didn't recognize the new `ASK_OUTCOME` line, or the real `llama-server`/`ggml-rpc` crash text, or HAProxy's own backend-down messages, as "worth flagging" at all — new patterns added to Sentinel's own `seed_patterns.py` (a separate repo/commit), verified live end-to-end against a real generated event: correctly flagged, then correctly matched to F-119 as the top KB suggestion (0.186 confidence) with F-122 and F-118 surfacing alongside it. Along the way, Sentinel's own test suite caught a genuine, pre-existing gap unrelated to this round's own changes — F-099/F-100/F-101/F-114 are referenced in this log's own prose but were never given their own `### F-NNN` section, so they've never actually been in Sentinel's KB — flagged to the user, not fixed (separate scope). A further real incident during this same investigation — a student's question stuck permanently on "Thinking…", a second attempt landing in an unusually long outage — led to F-125: the coordinator journal for the exact incident showed the F-119 watchdog fired correctly on schedule, but `systemctl restart llama-server.service` then silently cost a further 90 seconds waiting out systemd's own default `TimeoutStopSec`, since a genuinely deadlocked `llama-server` doesn't even respond to `SIGTERM` — confirmed live via the real "state stop-sigterm timed out, Killing" sequence. Fixed with an explicit `TimeoutStopSec=5` (llama-server has no state worth a graceful shutdown for), applied live to the running coordinator's own unit file without disturbing the already-healthy process. The "first bubble stuck forever, no message ever arrives" half of that same symptom remains only partially explained — flagged honestly as a narrow, real, not-fully-closed gap rather than claimed fixed. |
| v0.84 | 2026-09-23 | Paul Scott | Direct request, given the RPC deadlock's cost so far: keep `worker_peers`/RPC offloading fully intact for later (better hardware) while dropping it from the *current* deployment, since stourport can't carry the load as a shared dev/desktop machine either — a coordinator-alone (zero-worker) build on llwyn-y-groes instead. F-126: the coordinator's own `llama-server.service` `ExecStart` wasn't actually safe with `worker_peers = []` (empty `--rpc` argument merges with `-t`'s own flag, breaking startup outright) — fixed with a new conditional `local.llama_rpc_flags`. A second blocker, found only when actually submitting the real build: a `variables.tf` validation rule and a matching Ansible fail-fast task both still required at least one `worker_peers` entry, left over from when cross-host splitting was assumed mandatory — removed, with comments recording that empty is now a deliberately supported state. Verified end-to-end with a real destroy/rebuild (build `b853e131…`, coordinator on llwyn-y-groes, zero worker instances, real running unit confirmed via SSH to carry no `-ngl`/`--rpc` flags at all). Immediately after, the same stall symptom recurred with zero RPC involved — an in-progress investigation (not yet concluded) into whether this is the same `#28908` lock pattern or a separate, more general `llama-server`/`ggml` threading issue; two live-capture attempts so far both landed on the fallback "stalled, restarting" message rather than real generated content (one self-corrected after initially, wrongly, being reported as "real content generated then stalled" — the 125-character match was this codebase's own injected fallback text, not model output), and the user's own `print_timing` journal check confirmed zero real tokens were ever produced. A `/proc/<pid>/task/*/stack` capture during a live stall — the evidence needed to actually distinguish the two hypotheses — has not yet been obtained; port 8721 is bound to `127.0.0.1` on the coordinator VM itself, so this requires the user's own real-time SSH access to llwyn-y-groes to catch it in the moment before F-119's 120s watchdog restarts the process and destroys the evidence. |
| v0.85 | 2026-09-23 | Paul Scott | F-127: quantified the coordinator-alone deadlock properly via a direct Loki query against `verify_proxy.py`'s own structured `ASK_OUTCOME` logging rather than one-off manual tests — 13 of 14 real requests in the ~6 hours since the coordinator-alone rebuild stalled, every single one at 120.05–120.27s with zero answer characters, none partial. This is real evidence, not a race-prone edge case: a 100% failure rate with near-identical duration on uncontended, RPC-free first requests rules out #28908's own accept-loop starvation (which needs a second concurrent connection) as the sole or dominant cause, and points instead at a deterministic trigger inside `llama-server` itself, independent of RPC — model, build, or request-shape specific, not yet isolated. Caught one stall live end-to-end via `/sandbox/ask-status` polling (busy 15:39:38–15:40:18, released exactly at the matching `ASK_OUTCOME` line) and captured raw SSE byte timing for a separate attempt directly from stourport (zero bytes, not even a keep-alive ping, arrived before the connection was cut) — confirming `llama-server` never entered its own streaming code path at all, but not why. The one piece of evidence that would settle it — a live `/proc/<pid>/task/*/stack` capture during an in-progress stall — still requires the user's own real-time SSH access to llwyn-y-groes (confirmed again this round: stourport has no working key against that host) and remains the open next step. This finding explicitly revises F-126's own working assumption that dropping RPC would resolve the stalls — it did not. |
| v0.86 | 2026-09-23 | Paul Scott | Direct request, short-term: bake a dedicated, toggleable `claude-debug` sudo SSH user into every instance this template creates (`enable_claude_debug_access`, default true), reusing the existing `users:` mechanism ha-frontend-lb's own "ecs" user already proves works, rather than the CloudCore inter-instance keypair — independently revocable, distinguishable in auth logs, and (unlike that keypair) it turned out to work from stourport against instances on **either** host, llwyn-y-groes included, over the existing WireGuard peer mesh — superseding F-127's own note that llwyn-y-groes-hosted instances were unreachable from stourport. A real destroy/rebuild came back up with RPC enabled again (the UI's placement auto-recommendation pre-checked a worker peer) rather than reproducing F-126/F-127's coordinator-alone config — flagged, kept as-is per direct instruction rather than forcing another rebuild, since it gave a chance to test the *other* configuration. F-128: fired a real question and polled `/sandbox/ask-status` every 3s, capturing a full `/proc/<pid>/task/*/stack` snapshot on every busy poll — caught the exact moment (57s in) the coordinator's request-handling thread entered a `tcp_recvmsg`/`recvfrom` block and stayed there for the remaining 60+ seconds until the watchdog fired, then checked the worker's own `ggml-rpc-server` at the same moment and found its own connection-handling thread blocked in the identical stack, on the same single `ESTABLISHED` connection — a genuine mutual `recv()` deadlock, not the accept-loop-starves-a-second-connection mechanism #28908 literally describes (there was no second connection in this reproduction). The most direct, precise evidence this entire investigation arc has produced; root cause at the `ggml-rpc.cpp` protocol level still open. |
| v0.87 | 2026-09-23 | Paul Scott | The real, corrected resolution of the whole F-118→F-128 arc. F-129: a direct request to `llama-server`, bypassing `verify_proxy.py`'s watchdog entirely, with the real production-length system message, succeeded — 297.4s, HTTP 200, a complete real answer, with `llama-server`'s own `timings` object proving prefill alone (142.6s, 4.03 tok/s) already exceeds the old 120s stall timeout. Every past "deadlock"/`futex_wait` snapshot (F-118, F-128) was almost certainly genuinely slow, in-progress work, not a hang — this hardware/network/model combination is simply too slow for a 120s SLA, independent of RPC. Raw network throughput (20MB in 0.9s) rules out bandwidth; the coordinator's own host (stourport, load 2.87 on 4 real cores) is genuinely CPU-contended. Fixed by raising `generation_stall_timeout_seconds` to 420 in both the Terraform and Ansible copies (live-patched and confirmed on the running instance first). F-130: raising the timeout alone did NOT fix real usability — two live end-to-end reproductions through the actual student-facing path both show `llama-server` completing a full, real, successful answer (confirmed via its own journal) while `verify_proxy.py` simultaneously logs `answer_chars: 0`/`outcome: stalled` and shows the fallback message to the student. Ruled out, with direct evidence: network/RPC availability, connection failure, a crashed handler thread (the busy-gate's own `try/finally` logic proves it must still be alive), and a stuck service. Not resolved: two independent live `strace -f` sessions (one filtered, one full-syscall) during a confirmed-busy stall show zero syscalls of any kind from the thread that must, by the code's own logic, be alive and running `_relay_one_stream()`'s read loop — a genuine, unexplained contradiction, honestly flagged as open rather than claimed fixed. Recommended next step: internal debug logging inside `_relay_one_stream()` itself, since external `strace` has now failed twice to explain this thread's real behavior in this environment. |
| v0.88 | 2026-09-23 | Paul Scott | The actual conclusion of the whole F-118→F-130 investigation. Added `RELAY_DEBUG` (default off, toggleable per-instance), F-130's own recommended internal instrumentation — its timestamped log showed the exact mechanism directly: after the first legitimate 5s poll timeout, every subsequent one fired after only ~0.2s forever, never the full 5s again. Traced to CPython's own `socket.SocketIO.readinto()`: a genuine `socket.timeout` sets `self._timeout_occurred = True` **permanently**, and every later read on that file object raises immediately without ever calling `recv_into()` again — explaining both the symptom and why two earlier `strace` sessions saw zero syscalls (there was nothing left to trace). Confirmed with a clean, non-networked ~20-line local reproduction outside this whole deployment, both the bug and the fix. F-130 root-caused and fixed: raised `_open_upstream_completion()`'s connection timeout well above `GENERATION_STALL_TIMEOUT_S` so a real timeout essentially never fires, and switched `_relay_one_stream()`'s poll loop to `select()` on the raw socket, which never touches `_timeout_occurred` at all. F-131: separately moved the coordinator off contended stourport onto dedicated, idle llwyn-y-groes (dropping RPC, since the platform correctly forbids coordinator and worker sharing one peer, and the host's own 8-core budget was already claimed by the worker's flavor) — created a fresh, correctly-scoped security group directly on the peer via the project's own existing peer-aware `POST /v1/security-groups`. With both fixes deployed live to that new instance, a real `POST /sandbox/linux-ask` request — the actual student-facing path, no shortcuts — completed in 85.5s with a genuine, complete, correctly-formatted streamed answer and a natural `finish_reason: "stop"`. The first real answer this entire investigation ever produced through the real path. Both fixes committed to the templates for the next build. |
| v0.89 | 2026-09-23 | Paul Scott | Direct follow-up to close out the F-118→F-131 investigation properly: "in order to make sentinel more useful for debugging later should we do anything now?" Two gaps closed. (1) F-099/F-100/F-101 — real findings whose actual write-ups existed only as Document History prose, never given their own `### F-NNN` section, so `sentinel ingest-kb` had never actually seen them — recovered and given proper sections in place. F-114, by contrast, has no trace of its original content anywhere in this file's own git history beyond the one sentence that first flagged the gap; rather than invent content, that's now documented explicitly as an unrecoverable number, not silently skipped. KB count 127 → 131 (exactly +4) after re-ingesting, confirming all four now parse correctly. (2) Confirmed, directly against Sentinel's own matcher (not assumed): the routing of a "stalled" `ASK_OUTCOME` line to a specific finding comes entirely from `sentinel ingest-kb`'s own TF-IDF/embedding scoring over this file's real Symptom/Fix text, not from `seed_patterns.py`'s own trouble-detection regex (that file only decides whether a window is worth matching at all, unchanged and still correct). A synthetic "stalled" line, scored live against the freshly re-ingested KB, now matches F-130 first (confidence 0.226) ahead of the original F-119/F-125/F-122/F-118 diagnosis chain — meaning re-ingesting this log was already sufficient; no pattern code needed to change. `seed_patterns.py`'s own header comment updated to document this explicitly, so a future reader doesn't go looking for finding-routing logic in the wrong file. Sentinel's own test suite (35 tests) re-run clean after the change. |
| v0.90 | 2026-09-24 | Paul Scott | F-132: retrieval-grounding added for Linux Help, direct request after a live hallucination ("I don't feel comfortable with its current performance") — a new `kiwix-serve` instance (full Wikipedia nopic + ManKier man pages + ArchWiki) wired into `verify_proxy.py`'s `/sandbox/linux-ask` path only, with a small LLM query-extraction step ahead of the search (added after live testing showed raw conversational questions retrieve poorly against full-text search, a problem found and reported rather than shipped silently). Verified end-to-end against the real cloud deployment: the original motivating question ("explain ring 3 in detail") no longer produces the "other applications run in different rings" error, and the graceful-degradation path (kiwix stopped mid-flight) was independently confirmed live to still answer normally, ungrounded, rather than hang or fail. Two unrelated transient infrastructure failures hit building this (apt/dpkg conffile prompt, a small binary download timeout) — both caused by simultaneous heavy downloads contending for the same host-level package repo, fixed live via SSH rather than a full rebuild. |
| v0.91 | 2026-09-24 | Paul Scott | Direct report: a Linux Help answer stopped mid-sentence with no error shown, initial suspicion being conversation-history/context growth. F-133: root-caused instead, with the user's own direct access to the peer host (no SSH credentials of my own to physical hosts) — ruled out the CloudCore platform, the guest OS, and the physical host itself (`journalctl --list-boots` showed one continuous host uptime since 2026-09-12) in turn, then found the real mechanism: this peer runs guests under session-mode libvirt (`qemu:///session`, tied to the user's own desktop login), and the domain's own log showed `shutting down, reason=crashed` at the exact moment `cloudcore-api.service` (and `cloudcore-terminal.service`/`sentinel-ui.service`) were restarted — confirmed directly with the user as a deliberate `git pull` + restart to pick up the just-pushed F-132 fix. An entirely ordinary, previously-routine action turns out to crash any VM currently running on this specific peer; flagged as a real operational hazard rather than a code bug (`api/compute.py` already uses the correct persistent-domain API), not fixed at the infrastructure level without being asked. F-134, the direct in-scope mitigation: `runAsk()`'s streaming-read loop couldn't tell a truncated response (`reader.read()` resolving `done: true` on a dropped connection) from a genuinely complete one, so F-133's own crash was shown to the student as if it were a normal, finished answer — fixed by tracking whether the server's own closing `data: [DONE]` sentinel actually arrived, appending a clear notice when it didn't rather than silently standing in as the real answer. |
| v0.92 | 2026-09-24 | Paul Scott | Direct report: "i've destroyed and rebuilt the llm-chat environment and we are sitting at 503 for hours now." F-135 (RESOLVED): the same promtail dpkg-conffile-prompt failure first hit live during F-132's own build recurred identically on a genuinely fresh, from-scratch rebuild — proving it wasn't contention-dependent bad luck after all, but a deterministic template bug (`write_files` always writes `/etc/promtail/config.yml` before the declarative `packages:` phase installs promtail itself, so the interactive "keep the local version?" prompt always fires with no TTY to answer it, aborting the whole apt transaction and silently skipping every package listed after it — confirmed live: `libgomp1`/`promtail`/`dnsmasq`/`python3-paramiko` all completely missing, `llama-server` crash-looping 5184 times over several hours with no visible error beyond a permanent 503). Fixed properly this time — baked the already-proven one-line `--force-confold` fix into all three `examples/llm-chat` cloud-init templates (coordinator/worker/kiwix, both Terraform and Ansible copies) as a new first `runcmd` step, rather than live-patching the broken instance again and leaving every future rebuild to hit the same wall. Live-verified end-to-end on the actual broken coordinator before templating: packages installed cleanly, `llama-server` came up healthy, and the LB's own health check flipped `DOWN 1/2` → `UP` right on schedule. A separate, already-known, not-yet-addressed gap (`dnsmasq.service` failing on a port-53 conflict with `systemd-resolved`, affecting only the Firecracker sandbox terminal) was left open rather than silently folded into this fix. |
| v0.93 | 2026-09-24 | Paul Scott | Direct follow-up after confirming a grounded answer was accurate: "are we able to tell what resources the llama server used... did we use both the llm and then corroborate with man/wikipedia?" — honest answer at the time was only circumstantial (no success-path logging existed, only failures). Direct request to fix that properly: "I'd like to see the search term('s) for both linux and code logged and the references made/grounding requests entered into sentinel where a user can see/check what happened with the question/answer session" — confirmed via clarifying question this meant extending retrieval-grounding itself to the coding Ask panel, not just adding logging to Linux Help's existing scope. F-136: `_kiwix_search()` now returns structured references alongside its prompt string; grounding fires on both panels; a new `_log_grounding()` logs every completed ask (grounded or not) both to stdout (Loki, for free) and via a backgrounded push to a new Sentinel endpoint, modeled on `api/scheduler.py`'s own existing direct-push precedent to `/api/kb/import` rather than Sentinel's Loki-polling trouble-detection pipeline (the wrong shape for a plain per-request record). New `grounding_log` table + `/api/grounding-log` routes + a "Grounding" UI tab on the Sentinel side, following its own existing Knowledge Base tab's expand-row pattern. Caught and fixed a real bug before it shipped, not after: `code_verified` passed as a bare Python bool would have been silently coerced by SQLite's own TEXT-affinity rules into `"1"`/`"0"`, breaking the UI's boolean check — fixed by spelling it out as an explicit `"passed"`/`"failed"`/`null` string throughout. Verified live against the real coordinator: real questions through both panels produced accurate entries (Linux Help's own references stayed genuinely relevant; the coding panel's were noticeably weaker — real confirmation of the tradeoff already flagged before this was scoped in, not just theory), and graceful degradation was confirmed live too (Sentinel stopped entirely, a real question still completed normally with the expected non-fatal failure logged). |
| v0.94 | 2026-09-24 | Paul Scott | Direct follow-up to F-136: "could we benefit by checking that corpus first... and then grounding in the other areas?" — confirmed, gated on an explicit human marking system ("worked as expected" / "failed inspection, remove it from the corpus") plus a copy action for closer inspection elsewhere. F-137: `review_status` (unreviewed/approved/rejected) and `grounding_source` added to `grounding_log` via Sentinel's existing additive-migration mechanism; `POST /api/grounding-log/<id>/status` mirrors the existing suggestion-status route exactly; "rejected" excludes an entry from future reuse without deleting the row, matching how a dismissed suggestion already works. New `GET /api/grounding-log/match` (plain word-overlap scoring against approved-only entries, cheap and adequate for a small starting corpus — FTS5 confirmed available for later, not built speculatively now) is checked first by a new `_local_corpus_search()` in `verify_proxy.py`, with kiwix only tried on a miss — a local hit is labeled "Verified Q&A" in the prompt, keeping the same fact-check-and-reword framing rather than copying a stored answer verbatim. Verified live end-to-end against the real coordinator and the real production Sentinel DB: migration applied cleanly to existing data, an unreviewed entry confirmed never matched even with its own exact terms, then a real approved entry ("what does the du command do") correctly served a paraphrased follow-up question with kiwix skipped entirely and the model's answer genuinely reworded, not copied. |
| v0.95 | 2026-09-24 | Paul Scott | Direct follow-up: "are there any other downloadable resources for grounding... especially in the python field for the coding area?" — checked the real Kiwix catalog rather than guess, then "let's grab as much as we can... but only quality products... anything with flimsy reputation isn't required." F-138: added the official Python docs (~3.96GB), official PEPs, and a curated DevDocs bundle (python/numpy/pandas/django/flask/fastapi/matplotlib/scikit-learn/requests/jinja/click/pygame, ~110MB) to kiwix-serve — ~4.1GB total. Deliberately excluded video-transcript tutorial ZIMs (thin text content, a poor grounding fit regardless of the presenter's own real reputation) and non-Python language docs. Caught a real bug mid-download: the one-off bulk-download script was missing curl's `-C -` resume flag, so a stalled 3.96GB download was silently restarting from zero on every retry instead of resuming — caught by comparing file sizes across two check-ins, fixed, and confirmed the fix actually resumes correctly. Deployed live to the running kiwix instance without a rebuild (scp + a second sha256 pass on the remote side, byte-for-byte confirmed) and verified each new corpus is genuinely indexed with real, on-target search hits, not just present on disk. |
| v0.96 | 2026-09-24 | Paul Scott | Direct report: a student pasted a long `pip` error into Linux Help and got a confidently-wrong answer ("externally managed" explained backwards, plus a nonexistent `python3-pipx` package name — both confirmed wrong against a real Ubuntu box). F-139: traced to the server's own log (`search-term extraction failed... TimeoutError`) — a long pasted error pushes prefill time past the old 30s extraction timeout, and on timeout the fallback search pattern is the raw, still-long question, which finds nothing useful either way, so the request got zero grounding silently. Fixed: timeout raised 30→90s, a new 1000-char cap on what's sent to extraction specifically (the model still answers the student's full question regardless), and a visible note appended when truncation happens, telling the student plainly and recommending they paste just the most relevant part next time. Verified live with the exact original failing input: extraction now succeeds, response time dropped from timing out past 400s to 81s. Separately, live-confirmed (deliberately, by direct instruction, specifically testing the review-gate: "sometime I want to put things in the way to make sure we're keeping on track") that F-137's own known propagation risk is real, not just theoretical: the original wrong answer had been approved in Sentinel, so it was faithfully reused as grounding for the repeat question — `find_match()` worked exactly as designed, the human approval itself was simply wrong. Rejected via Sentinel's API once confirmed. Also added, same round: a "Clear session" button on the Linux Help panel, mirroring the coding panel's existing one, deliberately still a separate action. |
| v0.97 | 2026-09-24 | Paul Scott | Direct request: "can we include this repo's codebase... in the ask the model and linux help" — scoped to real source code only (Python/Terraform/Ansible/shell/Go), not ZIM packaging, with one hard requirement: "ensure it's easily updated... i'd would hate for it to be a monster chore." F-140: a new SQLite FTS5 index in Sentinel (not a ZIM archive, avoiding the rebuild-and-redeploy cost that would come with one), refreshed by a single `sentinel index-codebase <path>` command mirroring `ingest-kb`'s own shape — confirmed live a full rebuild costs ~2.5s (334 files, 3.3MB) so there's no reason to build incremental sync. Wired into `verify_proxy.py`'s existing check-order as a new middle tier between the human-approved Q&A corpus and kiwix. Caught and fixed two real FTS5 query bugs before calling it done: a hyphenated term ("force-confold") was silently read as a NOT-exclusion by FTS5's own query syntax, and even once quoted, the default AND-between-terms behavior made a realistic multi-word query come back empty since real code comments don't repeat a student's exact phrasing — fixed with quoted-and-OR'd terms plus bm25 ranking, matching kiwix's and F-137's own already-established best-effort search shape. Verified live end-to-end through the real Linux Help endpoint: a real question about this project's own promtail fix correctly grounded against `grounding_source: "codebase"`, citing real file paths, with an accurate answer specific to this project rather than generic knowledge. |
| v0.98 | 2026-09-24 | Paul Scott | Two hard, comparable live-UI test questions (one asked by the user directly through the UI, one asked back to test with) exposed F-140's own limitation: whole-file indexing diluted a large multi-topic file's best content under `bm25()`, and FTS5's `snippet()` landed on a call site instead of a function's own docstring. F-141: `codebase_index.py`'s `_chunk()` splits every file on blank-line boundaries into ≤800-char logical blocks (one universal chunker, not per-language parsers — this repo's own comments already sit directly above the code they explain), `codebase_fts` gains a `start_line` column for honest per-chunk provenance. Verified live with a genuinely mixed, honestly-reported result: direct endpoint tests confirm the ideal content (the offload function's own docstring, `variables.tf`'s real sizing story) now surfaces cleanly given the right query vocabulary, but the real end-to-end re-test came back with an honest "the reference material doesn't address this" rather than the ideal content, because that round's model-generated search-term extraction drifted toward the wrong vocabulary — chunking traded whole-file recall generosity for chunk-level precision, making search-term quality the new binding constraint, flagged as a follow-up lever rather than fixed this round. |
