...
**Verified by:** `tofu validate` (clean after fixing the stale comment reference), a direct Python string-substitution check confirming the rendered `ExecStart` line is correct and cleanly spaced in both the with-workers and zero-workers cases, and `yaml.safe_load()`/`jinja2.Environment().parse()` confirming the Ansible mirror's own YAML and Jinja2 syntax are both valid. Not yet verified against a real `tofu apply` with `worker_peers` genuinely empty — that's the next real build.
