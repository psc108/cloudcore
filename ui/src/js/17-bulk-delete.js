...
const _bulkMeta = {
  ...
  subnet:   { btn: 'subnet-delete-sel', selAll: 'subnet-sel-all', path: id => `/v1/subnets/${id}`,        reload: loadSubnets },
...
async function deleteSelected(type) {
  ...
  const label = type === 'instance' ? 'instance' : type === 'lb' ? 'load balancer' : type === 'subnet' ? 'subnet' : 'VPC';
