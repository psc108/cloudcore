...
# cloudcore-repo.service binds 192.168.100.1:8090 — $BRIDGE's own
...
if [ "${1:-}" != "--force" ] && systemctl is-active --quiet cloudcore-repo 2>/dev/null; then
  echo "cloudcore-repo.service is active on $BRIDGE (192.168.100.1:8090)." >&2
  ...
  echo "  sudo systemctl stop cloudcore-repo" >&2
  exit 1
fi
