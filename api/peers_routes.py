...
def peer_verdict(peer_id: str) -> str | None:
    ...
    now. Factored out of recommend_placement() below so
    api/scheduler.py's llm_ingest job can re-check a candidate worker
    peer's load at the moment it's actually about to build on it —
