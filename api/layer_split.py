...
def compute_offload_layers(model_path: Path, coordinator_stats: dict,
                            worker_stats_list: list[dict]) -> int | None:
    ...
    exact moment) — or None if it can't be computed (unreadable GGUF,
    no worker stats), in which case the caller should leave the
    template's own static default in place rather than block the
    build over this being best-effort."""
    ...
    return max(1, min(total_layers - 1, layers))
...
def maybe_apply(var_overrides: dict, schema: dict) -> None:
    ...
    layers = compute_offload_layers(model_path, coordinator_stats, worker_stats_list)
