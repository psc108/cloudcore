...
- **Anonymous Viewer access is on by default** (`[auth.anonymous]`,
  `org_role = Viewer`, same `grafana.ini` edit as the admin password
  ...
  chief among them). Viewer, not Editor/Admin — read-only, including
  Explore (Viewers get Explore access by default), no ability to
  save/delete anything; the admin login above still works
  independently for anyone who actually needs to administer this
  instance. `org_name` has to match the real default org's name
  exactly ("Main Org.", confirmed live via `GET /api/org`) or
...
| v0.31 | 2026-09-16 | Paul Scott | Grafana's host-level instance opens straight to Explore/dashboards now, no login screen, per direct instruction — `[auth.anonymous]` enabled in `grafana.ini` (`org_role = Viewer`, `org_name` matched exactly to the real default org, confirmed live via `GET /api/org`, or anonymous access silently falls back to requiring login). Read-only: Viewers get Explore access by default but can't save or delete anything; the existing admin login is unaffected for anyone who actually needs to administer the instance. `scripts/install.sh`'s printed summary and `README.md` updated to match — the admin password is now framed as "only needed to administer Grafana," not something a normal viewer needs at all. |
