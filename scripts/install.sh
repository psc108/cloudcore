...
echo ""
echo "    Once that's done, set up the host-level logging service (Loki +"
echo "    Grafana) — every example's own promtail agent ships to it, so any"
echo "    build (not just one specific example) has real log coverage:"
echo "      sudo bash api/setup-logging-service.sh"
echo "    Also not run automatically — needs the grafana/loki .debs the"
echo "    package-repo step above just cached."
