...
| 3A-14 | Failure test 5 — stop both Keystone nodes | LLD §3.3.1a test 5: genuine outage, `keystone-status.html` correctly shows `CRITICAL` | Done — correctly showed `CRITICAL` throughout, recovered cleanly to `OK` once both nodes were restarted (subject to the same stop/start reachability window as test 1 — see F-027) |
